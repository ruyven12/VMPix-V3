...
const MUSIC_VENUES_TIMEOUT_MS = 15000;
...
function setMusicPeopleIndexCollection(rows, stateName = "fallback") {
  ...
  musicPeopleIndexCollection = sourceRows.map((row) => {
    const rowKey = getMusicPersonCollectionKey(row);
    const archiveRow = rowKey ? musicPersonDetailArchiveRows.get(rowKey) : null;
    if (!archiveRow) {
      return row;
    }
    return {
      ...
      backend_record: {
        ...
        ...(archiveRow.backend_record || archiveRow),
      },
    };
  });
...
function normalizeMusicAlbumPhoto(photo, index = 0) {
  ...
  return {
    ...
    medium_url: String(photo?.medium_url || photo?.mediumUrl || "").trim(),
    large_url: String(photo?.large_url || photo?.largeUrl || "").trim(),
    date_time_original: String(photo?.date_time_original || photo?.dateTimeOriginal || photo?.DateTimeOriginal || "").trim(),
    caption,
...
function layoutLightboxInFrame() {
  lightboxInFrameLayoutFrame = 0;
  if (!lightboxScreen?.classList.contains("has-match-in-frame") || !lightboxImage || !lightboxPhoto) return;
  const stage = lightboxPhoto.parentElement;
  const gap = Number.parseFloat(getComputedStyle(stage).rowGap) || 0;
  const availableHeight = Math.max(1, stage.clientHeight - lightboxInFrameStrip.offsetHeight -
    (lightboxInFrameActions?.offsetHeight || 0) - gap * 2);
  const ratio = lightboxImage.naturalWidth && lightboxImage.naturalHeight
    ? lightboxImage.naturalWidth / lightboxImage.naturalHeight : 1;
  const width = Math.min(stage.clientWidth, availableHeight * ratio, lightboxImage.naturalWidth || stage.clientWidth);
  lightboxPhoto.style.setProperty("--in-frame-photo-width", `${width}px`);
  lightboxPhoto.style.setProperty("--in-frame-photo-height", `${width / ratio}px`);
  stage.style.setProperty("--in-frame-photo-center", `${lightboxPhoto.offsetTop + lightboxPhoto.offsetHeight / 2}px`);
}

function scheduleLightboxInFrameLayout() {
  if (!lightboxInFrameLayoutFrame) lightboxInFrameLayoutFrame = requestAnimationFrame(layoutLightboxInFrame);
}

function syncLightboxInFrame(tileDataset = null) {
  const route = typeof getRouteFromUrl === "function" ? getRouteFromUrl() : null;
  const enabled = Boolean(tileDataset?.galleryInFrameTags && tileDataset.wrestlingMatchRef &&
    typeof isWrestlingMatchDetailPhotoRoute === "function" && isWrestlingMatchDetailPhotoRoute(route));
  lightboxScreen?.classList.toggle("has-match-in-frame", enabled);
  if (!enabled) {
    cancelLightboxPhotoDownload();
    if (lightboxInFrameStrip) { lightboxInFrameStrip.hidden = true; lightboxInFrameStrip.querySelector("p").textContent = ""; }
    if (lightboxInFrameActions) {
      lightboxInFrameActions.hidden = true;
      const download = lightboxInFrameActions.querySelector("a");
      ["href", "download", "target", "rel"].forEach((name) => download.removeAttribute(name));
    }
    lightboxInFrameResizeObserver?.disconnect();
    cancelAnimationFrame(lightboxInFrameLayoutFrame);
    lightboxInFrameLayoutFrame = 0;
    lightboxPhoto?.style.removeProperty("--in-frame-photo-width");
    lightboxPhoto?.style.removeProperty("--in-frame-photo-height");
    lightboxPhoto?.parentElement?.style.removeProperty("--in-frame-photo-center");
    return;
  }
  if (!lightboxInFrameStrip) {
    lightboxInFrameStrip = document.createElement("section");
    lightboxInFrameStrip.className = "v3-lightbox-in-frame";
    lightboxInFrameStrip.setAttribute("aria-label", "Photo caption tags");
    const label = document.createElement("span");
    label.className = "v3-lightbox-in-frame__label";
    label.textContent = "IN FRAME";
  ...
  let tags = [];
  try { tags = JSON.parse(tileDataset.galleryInFrameTags); } catch { /* Missing caption remains unknown. */ }
...
function setLightboxActivePhoto(index, options = {}) {
  ...
  syncLightboxInFrame(tileDataset);
...
function getOptionalMusicPeopleCountValue(...values) {
  const foundValue = values.find((value) => {
    if (value === null || typeof value === "undefined" || value === "") {
      return false;
    }
  ...
  const parsedValue = Number.parseInt(foundValue, 10);
...
function normalizeMusicPeopleIndexRow(person) {
  ...
  const appearances = getOptionalMusicPeopleCountValue(
    source.appearances,
...
function getMusicPersonTaggedShowAlbumId(show) {
  return getMusicPeopleText(show?.album_id || show?.albumId || show?.gallery_id || show?.galleryId);
}

function getMusicPersonTaggedShowVenue(show) {
  const directVenue = getMusicPeopleText(show?.venue);
  if (isReadableMusicVenueName(directVenue)) {
    return directVenue;
  }

  const matchedShow = findMusicShowById(show?.show_id || show?.showId);
  return getMusicShowVenueName(matchedShow) || "Venue Pending";
}

function getMusicPersonBandRouteSlug(band) {
  return createBandSlug(getBandId(band) || band?.band_id || band?.id || band?.slug || band?.name || band);
}

function getMusicPersonTaggedShowMatchedShow(show) {
...
function getMusicPersonTaggedShowBandSlug(show, associatedBands = [], matchedShow = null) {
  const showBandSlugs = [
    show?.band_id,
    show?.bandId,
    show?.band_slug,
  ...
  const matchedBand = associatedBands.find((band) => {
    const bandSlug = getMusicPersonBandRouteSlug(band);
    return bandSlug && (
      showBandSlugs.includes(bandSlug) ||
      (matchedShow?.bandSlugs || []).some((showSlug) => createBandSlug(showSlug) === bandSlug) ||
      (matchedShow?.bandNames || []).some((showBand) => createBandSlug(showBand) === bandSlug)
    );
  });
  if (matchedBand) {
    return getMusicPersonBandRouteSlug(matchedBand);
  }

  if (associatedBands.length === 1) {
    return getMusicPersonBandRouteSlug(associatedBands[0]);
  }

  return showBandSlugs[0] || createBandSlug((matchedShow?.bandSlugs || [])[0] || (matchedShow?.bandNames || [])[0]);
}

function getMusicPersonTaggedShowAlbumKey(album) {
  return getMusicPeopleText(
    album?.album_id ||
    album?.albumId ||
    album?.album_key ||
...
function getMusicPersonTaggedShowAlbum(show, matchedShow, associatedBands = []) {
  const albums = getMusicShowSmugAlbums(matchedShow);
  if (albums.length === 0) {
    return null;
  }
  ...
  if (albumId) {
    const matchingAlbum = albums.find((album) => getMusicPersonTaggedShowAlbumKey(album) === albumId);
    if (matchingAlbum) {
      return matchingAlbum;
    }
  }

  const bandSlug = getMusicPersonTaggedShowBandSlug(show, associatedBands, matchedShow);
  return albums.find((album) => {
    return [getMusicShowAlbumBandId(album), getMusicShowAlbumBandName(album), album?.lineupBand, album?.lineup_band, album?.band_folder]
      .map(createBandSlug)
      .some((albumBandSlug) => albumBandSlug && albumBandSlug === bandSlug);
  }) || (albums.length === 1 ? albums[0] : null);
}
...
function getMusicPersonTaggedShowSetCode(show, matchedShow = null, album = null) {
  return normalizeSetCode(
    ...
    matchedShow?.date_folder ||
    matchedShow?.dateFolder ||
    matchedShow?.setCode ||
    matchedShow?.set_code ||
    getSetCodeFromDateValue(show?.rawDate || show?.date || show?.show_date || show?.showDate || show?.eventDate || show?.formattedDate) ||
    getMusicShowRouteCode(matchedShow)
  );
}

function getMusicPersonTaggedShowRoute(show, associatedBands = []) {
  const matchedShow = getMusicPersonTaggedShowMatchedShow(show);
  const matchedAlbum = getMusicPersonTaggedShowAlbum(show, matchedShow, associatedBands);
  if (matchedShow && matchedAlbum) {
    const albumRoute = getMusicShowAlbumSetRoute(matchedShow, matchedAlbum);
    if (albumRoute) {
      return albumRoute;
    }
  }
  ...
  const bandSlug = getMusicPersonTaggedShowBandSlug(show, associatedBands, matchedShow);
  const setCode = getMusicPersonTaggedShowSetCode(show, matchedShow, matchedAlbum);
  return bandSlug && setCode ? getSetRouteUrl(bandSlug, setCode) : "";
}
...
function normalizeMusicPersonTaggedShowRow(show, index, associatedBands = []) {
  ...
  return {
    ...
    title,
    venue: getMusicPersonTaggedShowVenue(show),
    ...
    showRoute: getMusicPersonTaggedShowRoute(show, associatedBands),
    setCode: getMusicPersonTaggedShowSetCode(show, matchedShow),
...
function getMusicPersonTaggedShowsFromMatchedPhotos(source, associatedBands = []) {
  ...
  photos.forEach((photo) => {
    ...
    if (!groups.has(groupKey)) {
      ...
      groups.set(groupKey, {
        ...
        title,
        venue: getMusicPersonTaggedShowVenue(photo),
  ...
  return musicShowsSetsCollection
    ...
    .map((show, index) => {
      ...
      return {
        ...
        showRoute: getMusicPersonTaggedShowRoute(show, associatedBands),
        setCode: getMusicPersonTaggedShowSetCode(show, show),
...
function getMusicPersonCaptionNameParts(photo) {
  return getMusicPeopleText(photo?.caption || photo?.Caption)
    .split(";")
    .map((name) => name.trim())
    .filter(Boolean);
}

function doesMusicPersonCaptionMatch(photo, personName) {
  const normalizedPersonName = getMusicPeopleText(personName).toLowerCase();
  if (!normalizedPersonName) {
    return false;
  }
  return getMusicPersonCaptionNameParts(photo)
    .some((name) => name.toLowerCase() === normalizedPersonName);
}

function getMusicPersonShowCardById(showId) {
  const normalizedShowId = getMusicPeopleText(showId);
  if (!personDetail || !normalizedShowId) {
    return null;
  }
  return Array.from(personDetail.querySelectorAll(".person-show-card"))
...
function createMusicPersonShowCard(show, personName) {
  ...
  venue.textContent = [show.venue, show.location].map(getMusicPeopleText).filter(Boolean).join(" - ");
  ...
  const showRoute = getMusicPeopleText(show.showRoute);
  if (showRoute) {
    action.dataset.showRoute = showRoute;
    action.addEventListener("click", (event) => {
      ...
      navigateToRoute(showRoute, {
        historyState: {
          ...
          personDetailUrl: currentRoute.name === "person-detail" ? currentRoute.canonicalUrl : getMusicPersonRouteUrl(getMusicPeopleText(show.personId)),
        },
      });
    });
  } else {
...
function renderMusicPersonDetail(data) {
  ...
  taggedHeader.append(taggedTitle);
  ...
  hydrateMusicPersonTaggedShowPhotos(data);
}
...
function createMusicPeopleRow(person) {
  ...
  const hasAppearances = Number.isFinite(person.appearances);
  const hasPhotos = Number.isFinite(person.photos);
  const row = document.createElement("button");
  ...
  row.setAttribute(
    ...
    [
      ...
      hasAppearances && (!isFriend || person.appearances > 0) ? formatMusicPeopleCount(person.appearances, "Appearance") : "",
      hasPhotos && (!isFriend || person.photos > 0) ? formatMusicPeopleCount(person.photos, "Photo") : "",
    ].filter(Boolean).join(", ")
  );
  ...
  if (hasAppearances && (!isFriend || person.appearances > 0)) {
  ...
  if (hasPhotos && (!isFriend || person.photos > 0)) {
    const photos = document.createElement("span");
...
function getMusicShowImageUrlValue(source, options = {}) {
  ...
  for (const key of keys) {
    const imageUrl = String(source[key] || "").trim();
...
function addMusicShowImageUrlsFromObject(urls, source, options = {}) {
  addMusicShowImageUrl(urls, getMusicShowImageUrlValue(source, options));
}
...
function getMusicShowPhotoCaption(source) {
  if (!source || typeof source !== "object") {
    return "";
  }

  return String(source.caption || source.Caption || "").trim();
}

function getMusicShowPhotoDedupeKey(source, imageUrl) {
  if (!source || typeof source !== "object") {
    return imageUrl;
  }
...
function addMusicShowAlbumPhotoCandidate(candidates, source, options = {}) {
  const imageUrl = getMusicShowImageUrlValue(source, options);
  if (!imageUrl || !isMusicShowPosterImageSrc(imageUrl)) {
    return;
  }
  ...
  candidates.push({
    url: imageUrl,
    caption: getMusicShowPhotoCaption(source),
    dedupeKey: getMusicShowPhotoDedupeKey(source, imageUrl),
  });
}

function addMusicShowAlbumPhotoCandidatesFromCollection(candidates, collection) {
  if (typeof collection === "string" && collection.trim().startsWith("[")) {
    try {
      addMusicShowAlbumPhotoCandidatesFromCollection(candidates, JSON.parse(collection));
    } catch (error) {
      return;
    }
    return;
  ...
  collection.forEach((item) => addMusicShowAlbumPhotoCandidate(candidates, item, { includeGenericUrl: true }));
}

function getMusicShowAlbumPhotoCandidates(show, album, photos = null) {
  const candidates = [];
  const photoSource = Array.isArray(photos) && photos.length > 0 ? photos : [
  ...
  if (Array.isArray(photos) && photos.length > 0) {
    photos.forEach((photo) => addMusicShowAlbumPhotoCandidate(candidates, photo, { includeGenericUrl: true }));
  } else {
    photoSource.forEach((collection) => addMusicShowAlbumPhotoCandidatesFromCollection(candidates, collection));
  }

  if (candidates.length === 0) {
    addMusicShowAlbumPhotoCandidate(candidates, album, { includeGenericUrl: true });
    [
      show?.cover_image_url,
      show?.coverImageUrl,
      show?.poster,
    ].forEach((value) => addMusicShowAlbumPhotoCandidate(candidates, value, { includeGenericUrl: true }));
  }

  const seen = new Set();
  return candidates.filter((candidate) => {
    const key = candidate.dedupeKey || candidate.url;
    if (!key || seen.has(key)) {
      return false;
    }
...
function getMusicShowAlbumImageCandidates(show, album) {
  return getMusicShowAlbumPhotoCandidates(show, album).map((candidate) => candidate.url);
}

function shuffleMusicShowPreviewItems(items) {
...
function normalizeMusicShowCaptionSearchText(value) {
  return String(value || "")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\s+/g, " ")
...
function getMusicShowAlbumMemberNames(album) {
  const bandId = getMusicShowAlbumBandId(album);
  const bandRecord = findBandById(bandId) || findMusicBandByName(getMusicShowAlbumBandName(album));
  const personnel = getBandDetailPersonnel(bandRecord);
  const sources = [
    album?.members,
    album?.core_members,
    album?.coreMembers,
...
function doesMusicShowCaptionMatchMember(caption, memberName) {
  const captionText = normalizeMusicShowCaptionSearchText(caption);
  const memberText = normalizeMusicShowCaptionSearchText(memberName);
  if (!captionText || !memberText) {
    return false;
  }

  if (captionText.includes(memberText)) {
    return true;
  }

  const memberTerms = memberText.split(" ").filter((term) => term.length > 2);
  if (memberTerms.length <= 1) {
    return memberTerms.length === 1 && captionText.split(" ").includes(memberTerms[0]);
  }

  const captionTerms = new Set(captionText.split(" "));
  return memberTerms.every((term) => captionTerms.has(term));
}

function getMusicShowCaptionMatchedPreviewCandidates(candidates, album) {
  const memberNames = getMusicShowAlbumMemberNames(album);
  ...
  memberNames.forEach((memberName) => {
    const matches = candidates.filter((candidate) => {
      ...
      return key &&
        ...
        doesMusicShowCaptionMatchMember(candidate.caption, memberName);
    });
    const match = shuffleMusicShowPreviewItems(matches)[0];
    if (!match) {
      return;
    }

    selected.push(match);
    selectedKeys.add(match.dedupeKey || match.url);
  });

  return selected;
}

function selectMusicShowAlbumPreviewImages(show, album, photos = null) {
  const candidates = getMusicShowAlbumPhotoCandidates(show, album, photos);
  if (candidates.length <= 1) {
    return candidates.length === 1 ? [candidates[0].url] : [SET_GALLERY_NO_POSTER_IMAGE_SRC];
  }

  const captionMatches = getMusicShowCaptionMatchedPreviewCandidates(candidates, album);
  const selected = captionMatches.slice(0, SHOW_SET_CARD_PREVIEW_IMAGE_LIMIT);
  const selectedKeys = new Set(selected.map((candidate) => candidate.dedupeKey || candidate.url));
  const remaining = candidates.filter((candidate) => !selectedKeys.has(candidate.dedupeKey || candidate.url));

  selected.push(...shuffleMusicShowPreviewItems(remaining).slice(0, SHOW_SET_CARD_PREVIEW_IMAGE_LIMIT - selected.length));
  if (selected.length === 0) {
    selected.push(...shuffleMusicShowPreviewItems(candidates).slice(0, SHOW_SET_CARD_PREVIEW_IMAGE_LIMIT));
  }
...
function getRandomizedMusicShowAlbumPreviewImages(show, album) {
  return selectMusicShowAlbumPreviewImages(show, album);
}
...
function getMusicShowAlbumPreviewAlbumId(album) {
  return String(album?.album_id || album?.albumId || album?.gallery_id || album?.galleryId || "").trim();
}

function getUniqueShowSetPreviewUrls(urls) {
  return Array.from(new Set((Array.isArray(urls) ? urls : [])
    .map((url) => String(url || "").trim())
    .filter((url) => url && isMusicShowPosterImageSrc(url))));
}

function setShowSetCardPreviewUrls(card, urls) {
...
function ensureShowSetCardPreviewAlbumPhotos(card) {
  ...
  if (
    !preview ||
    preview.albumPhotosLoaded ||
    preview.albumPhotosLoading ||
    !preview.albumId ||
  ...
  requestMusicShowAlbumPreviewUrls(preview)
    ...
    .finally(() => {
      preview.albumPhotosLoading = false;
    });
}
...
function updateShowSetCardPreviewPlayback(card) {
  ...
  if (!preview.timerId) {
    ...
    }, preview.intervalMs || SHOW_SET_CARD_PREVIEW_INTERVAL_MIN_MS);
  }
}
...
function cleanupShowSetCardPreviews() {
  showSetPreviewCards.forEach((card) => {
    const preview = card?.__showSetPreview;
    stopShowSetCardPreview(card);
    if (preview?.visibilityObserver) {
      preview.visibilityObserver.disconnect();
      preview.visibilityObserver = null;
    }
  });
  showSetPreviewCards.clear();
}
...
function registerShowSetCardPreview(card) {
  const preview = card?.__showSetPreview;
  if (!preview || (preview.urls.length <= 1 && !preview.albumId)) {
    return;
  }
...
function createShowDetailAlbumCard(show, album, index) {
  ...
  const albumId = getMusicShowAlbumPreviewAlbumId(album);
  const previewImages = (isShowSetPreviewReducedMotion()
    ? getRandomizedMusicShowAlbumPreviewImages(show, album).slice(0, 1)
    : getRandomizedMusicShowAlbumPreviewImages(show, album));
  ...
  card.__showSetPreview = {
    ...
    currentIndex: 0,
    timerId: 0,
    intervalMs: SHOW_SET_CARD_PREVIEW_INTERVAL_MIN_MS + Math.floor(Math.random() * SHOW_SET_CARD_PREVIEW_INTERVAL_JITTER_MS),
    isCarouselActive: false,
    isVisible: false,
    albumPhotosLoaded: false,
