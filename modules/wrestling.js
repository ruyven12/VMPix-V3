...
const HALL_OF_CHAMPIONS_PEOPLE_ARCHIVE_CACHE_KEY = "vmpix:hall-of-champions:people-archive:v1";
const HALL_OF_CHAMPIONS_PEOPLE_ARCHIVE_CACHE_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000;
...
let hallOfChampionsPeopleArchiveNetworkComplete = false;
...
let hallCrusadesCampaignRecordPreparationFrame = 0;
let hallCrusadesCampaignRecordPreparationTimer = 0;
let hallCrusadesCampaignPreparedRecordShowId = "";
  ...
  wrestlingShowsRequest = fetchWrestlingShowsPage(1, controller?.signal)
    ...
    .then((payload) => {
      ...
      if (typeof isWrestlingMatchDetailRoute === "function" && isWrestlingMatchDetailRoute(route)) {
        ...
        showWrestlingPersonDossierRoute(route.personId || route.params?.personId);
      }
...
function isHallCrusadesCampaignLockActive() {
...
function clearHallCrusadesCampaignChargeTimers(options = {}) {
  if (typeof window === "undefined") {
    return;
  }

  clearHallCrusadesCampaignRecordPreparationRequest();
  clearHallCrusadesCampaignRoutePromotion(options.keepRoutePromotion ? { keepPending: true } : {});
...
function clearHallCrusadesCampaignRecordPreparationRequest() {
  if (typeof window === "undefined") {
    return;
  }

  window.cancelAnimationFrame(hallCrusadesCampaignRecordPreparationFrame);
  window.clearTimeout(hallCrusadesCampaignRecordPreparationTimer);
  hallCrusadesCampaignRecordPreparationFrame = 0;
  hallCrusadesCampaignRecordPreparationTimer = 0;
}
...
function clearHallCrusadesCampaignLock(options = {}) {
  clearHallCrusadesCampaignChargeTimers(options);
...
function closeHallCrusadesCampaignLockControls() {
...
function setHallCrusadesCampaignMobileCoverGeometry(variableTarget = wrestlingShowsShell, record = getHallCrusadesCampaignActiveRecord()) {
  if (!variableTarget || typeof window === "undefined") {
    return false;
  }

  const resolvedRect = record?.getBoundingClientRect?.();
  if (!resolvedRect?.width || !resolvedRect?.height) {
    return false;
  }
  ...
  setVar("--hall-crusades-campaign-target-x", resolvedRect.left);
  setVar("--hall-crusades-campaign-target-y", resolvedRect.top);
  setVar("--hall-crusades-campaign-target-width", resolvedRect.width);
  setVar("--hall-crusades-campaign-target-height", resolvedRect.height);
...
function queueHallCrusadesCampaignMobileCover(next) {
  ...
  if (!setHallCrusadesCampaignMobileCoverGeometry(wrestlingShowsShell, record)) {
    next?.();
    return false;
  }
  ...
  if (!isReducedMotion && record) {
    const coverListener = (event) => {
      if (event.target === record && coverTransitionProperties.has(event.propertyName)) {
        completeCover();
      }
    };
    hallCrusadesCampaignRoutePromotionWait = { element: record, listener: coverListener };
    record.addEventListener("transitionend", coverListener);
  }

  window.clearTimeout(hallCrusadesCampaignMobileCoverTimer);
  hallCrusadesCampaignMobileCoverTimer = window.setTimeout(
    completeCover,
    coverDuration + (isReducedMotion ? 0 : HALL_CRUSADES_CAMPAIGN_ROUTE_PROMOTION_FALLBACK_BUFFER_MS)
  );
  return true;
}
...
function getHallCrusadesCampaignRecordShowId(show) {
  return show?.showId || show?.eventId || hallCrusadesCampaignLockShowId || "";
}
...
function getHallCrusadesCampaignRoutePath(url = window.location.href) {
  if (typeof window === "undefined") {
    return "";
  }
  ...
  try {
    ...
    return typeof normalizeRoutePath === "function"
      ? normalizeRoutePath(routeUrl.pathname)
      : routeUrl.pathname.replace(/\/+$/, "") || "/";
  } catch (error) {
    return "";
  }
}

function isHallCrusadesCampaignRecordProductionRoute(path = getHallCrusadesCampaignRoutePath()) {
  return Boolean(path && path.startsWith(`${routePaths.wrestlingShows}/`));
}

function syncHallCrusadesCampaignRecordEngineBar(path = getHallCrusadesCampaignRoutePath()) {
  if (!isHallCrusadesCampaignRecordProductionRoute(path)) {
    return;
  }
  if (typeof clearHallEngineTitleMorph === "function") {
...
function scheduleHallCrusadesCampaignRecordEngineBarSync(path = getHallCrusadesCampaignRoutePath()) {
  syncHallCrusadesCampaignRecordEngineBar(path);
...
function getHallCrusadesCampaignExistingRecordDetailHost() {
  return wrestlingShowsShell?.querySelector?.("[data-hall-crusades-campaign-record-detail]") || null;
}

function getHallCrusadesCampaignRoutePromotionTransitionMs(element) {
  ...
  const durations = String(style.transitionDuration || "").split(",").map(parseTransitionTime);
  const delays = String(style.transitionDelay || "").split(",").map(parseTransitionTime);
  return durations.reduce((max, duration, index) => Math.max(max, duration + (delays[index] || delays[0] || 0)), 0);
}

function getHallCrusadesCampaignRecordScrollState() {
  const host = getHallCrusadesCampaignExistingRecordDetailHost();
  if (!host) {
    return null;
  }

  const readScrollState = (selector) => {
    const element = host.querySelector(selector);
    return element
      ? { selector, left: element.scrollLeft || 0, top: element.scrollTop || 0 }
      : null;
  };

  return [
    readScrollState(".wrestling-show-prototype-surface"),
    readScrollState(".wrestling-show-prototype-encounters"),
    readScrollState(".wrestling-show-prototype-encounter-list"),
  ].filter(Boolean);
}

function applyHallCrusadesCampaignRecordScrollState(scrollState, targetShell = wrestlingShowDetailShell) {
  if (!targetShell || !Array.isArray(scrollState)) {
    return;
  }

  scrollState.forEach(({ selector, left, top }) => {
    const element = targetShell.querySelector(selector);
    if (element) {
...
function isHallCrusadesCampaignRoutePromotionReady(show, routeUrl) {
  const recordShowId = getHallCrusadesCampaignRecordShowId(show);
  const host = getHallCrusadesCampaignExistingRecordDetailHost();
  return Boolean(
    typeof navigateToRoute === "function" &&
    routeUrl &&
    recordShowId &&
    wrestlingShowsDataState === "live" &&
    !wrestlingShowsRequest &&
    isHallCrusadesCampaignLockActive() &&
    isHallCrusadesCampaignExpanded &&
    hallCrusadesCampaignResolutionState === "matches" &&
    isHallCrusadesCampaignRecordResolutionPrepared(show) &&
    host &&
    host.getAttribute("aria-hidden") !== "true" &&
    host.querySelector('[data-hall-crusades-resolution-part="identity"]') &&
    host.querySelector('[data-hall-crusades-resolution-part="metadata"]') &&
    host.querySelector('[data-hall-crusades-resolution-part="matches"]') &&
    getHallCrusadesCampaignRoutePath() === routePaths.wrestlingShows &&
    getHallCrusadesCampaignRoutePath(routeUrl).startsWith(`${routePaths.wrestlingShows}/`) &&
    document.querySelector("[data-hall-crusades-firelight]") &&
    document.querySelector("[data-hall-crusades-haze]") &&
    document.querySelector('[data-hall-crusades-cinders="distant"]')
  );
}

function promoteHallCrusadesCampaignResolvedRecord(show, routeUrl) {
  if (!isHallCrusadesCampaignRoutePromotionReady(show, routeUrl)) {
    clearHallCrusadesCampaignRoutePromotion();
    return;
  }

  if (isHallCrusadesCampaignMobileCoverViewport() && wrestlingShowsShell?.dataset.hallCrusadesCampaignPerimeterFaded === "true") {
    setHallCrusadesCampaignDirectRouteHandoffShellStyles();
  }
...
function scheduleHallCrusadesCampaignRoutePromotion() {
  ...
  const routeUrl = getWrestlingShowRouteUrl(show);
  if (!isHallCrusadesCampaignRoutePromotionReady(show, routeUrl)) {
    return;
  }

  clearHallCrusadesCampaignRoutePromotion();
  const matchesPart = getHallCrusadesCampaignExistingRecordDetailHost()?.querySelector('[data-hall-crusades-resolution-part="matches"]');
...
function setHallCrusadesCampaignDirectRouteHandoffShellStyles() {
  const routeShell = wrestlingShowDetailShell?.closest?.(".site-shell") || wrestlingShowsShell?.closest?.(".site-shell") || null;
  if (routeShell) {
    routeShell.dataset.hallCrusadesCampaignDirectRouteHandoff = "true";
  }
  ...
  setStyles(wrestlingShowDetailShell, {
    transition: "none",
    opacity: "1",
    visibility: "visible",
    "pointer-events": "auto",
...
function clearHallCrusadesCampaignDirectRouteHandoffShellFlag() {
  const routeShell = wrestlingShowDetailShell?.closest?.(".site-shell") || wrestlingShowsShell?.closest?.(".site-shell") || null;
  delete routeShell?.dataset.hallCrusadesCampaignDirectRouteHandoff;
}


function clearHallCrusadesCampaignDirectRouteHandoffShellStyles() {
  [wrestlingShowsShell, wrestlingShowDetailShell].forEach((element) => {
    if (!element) {
      return;
    }
    ["transition", "opacity", "visibility", "pointer-events", "transform"].forEach((property) => {
      element.style.removeProperty(property);
    });
  });
}

function queueHallCrusadesCampaignDirectRouteHandoffStyleCleanup() {
  if (typeof window === "undefined") {
    clearHallCrusadesCampaignDirectRouteHandoffShellStyles();
    clearHallCrusadesCampaignDirectRouteHandoffShellFlag();
    return;
  }
...
function completeHallCrusadesCampaignDirectRoutePromotion(show, surface) {
  if (!surface || !wrestlingShowsShell || typeof window === "undefined") {
    return false;
  }

  const historyState = window.history?.state || {};
  const recordShowId = getHallCrusadesCampaignRecordShowId(show);
  const lockedShowId = wrestlingShowsShell.dataset.hallCrusadesCampaignLockShowId || "";
  const didFadePerimeter = wrestlingShowsShell.dataset.hallCrusadesCampaignPerimeterFaded === "true";
  if (!historyState.fromHallCrusadesCampaignRecord || !didFadePerimeter || !recordShowId || lockedShowId !== recordShowId) {
    return false;
  }

  setHallCrusadesCampaignDirectRouteHandoffShellStyles();
  clearHallCrusadesCampaignLock({ keepRoutePromotion: true });
  scheduleHallCrusadesCampaignRecordEngineBarSync();
  clearHallCrusadesCampaignRoutePromotion();
  queueHallCrusadesCampaignDirectRouteHandoffStyleCleanup();
  return true;
}

function completeHallCrusadesCampaignRoutePromotion(show, surface) {
  if (!surface) {
    return;
  }

  if (!hallCrusadesCampaignRoutePromotion) {
    completeHallCrusadesCampaignDirectRoutePromotion(show, surface);
    return;
  }

  const recordShowId = getHallCrusadesCampaignRecordShowId(show);
  if (recordShowId !== hallCrusadesCampaignRoutePromotion.recordShowId) {
    clearHallCrusadesCampaignRoutePromotion();
    return;
  }
  ...
  hallCrusadesCampaignRoutePromotionCleanupFrame = window.requestAnimationFrame(() => {
    applyHallCrusadesCampaignRecordScrollState(scrollState);
    syncHallCrusadesCampaignRecordEngineBar();
    hallCrusadesCampaignRoutePromotionCleanupFrame = window.requestAnimationFrame(() => {
      hallCrusadesCampaignRoutePromotionCleanupFrame = 0;
      applyHallCrusadesCampaignRecordScrollState(scrollState);
      syncHallCrusadesCampaignRecordEngineBar();
...
function clearHallCrusadesCampaignRecordResolutionState() {
  if (typeof window !== "undefined") {
    hallCrusadesCampaignResolutionTimers.forEach((timer) => window.clearTimeout(timer));
  }
  hallCrusadesCampaignResolutionTimers = [];
  hallCrusadesCampaignResolutionState = "";
  wrestlingShowsShell?.querySelector?.("[data-hall-crusades-campaign-record-detail]")?.setAttribute("aria-hidden", "true");
  if (wrestlingShowsShell) {
...
function clearHallCrusadesCampaignRecordResolution() {
  clearHallCrusadesCampaignRecordResolutionState();
  hallCrusadesCampaignPreparedRecordShowId = "";
  wrestlingShowsShell?.querySelector?.("[data-hall-crusades-campaign-record-detail]")?.remove();
}

function renderHallCrusadesCampaignRecordResolution(showOverride = null) {
  const show = showOverride || getHallCrusadesCampaignSelectedShow();
  ...
  host.dataset.hallCrusadesCampaignRecordShowId = recordShowId;
  ...
  hallCrusadesCampaignPreparedRecordShowId = recordShowId;
  return true;
}

function isHallCrusadesCampaignRecordResolutionPrepared(show = getHallCrusadesCampaignSelectedShow()) {
  const recordShowId = getHallCrusadesCampaignRecordShowId(show);
  const host = wrestlingShowsShell?.querySelector?.("[data-hall-crusades-campaign-record-detail]");
  return Boolean(
    recordShowId &&
    hallCrusadesCampaignPreparedRecordShowId === recordShowId &&
    host?.dataset?.hallCrusadesCampaignRecordShowId === recordShowId &&
    host.querySelector(".wrestling-show-prototype-surface")
  );
}

function prepareHallCrusadesCampaignRecordResolution(show = getHallCrusadesCampaignSelectedShow()) {
  if (!isHallCrusadesShowsVariantActive() || isHallCrusadesCampaignExpanding || isHallCrusadesCampaignExpanded) {
    return false;
  }
  ...
  if (isHallCrusadesCampaignRecordResolutionPrepared(show)) {
    return true;
  }

  return renderHallCrusadesCampaignRecordResolution(show);
}

function queueHallCrusadesCampaignRecordPreparation(show = getHallCrusadesCampaignSelectedShow(), delay = 180) {
  if (typeof window === "undefined" || !isHallCrusadesShowsVariantActive() || !show) {
    return;
  }
  ...
  const prepare = () => {
    ...
    prepareHallCrusadesCampaignRecordResolution(show);
  };

  if (isHallCrusadesCampaignReducedMotion() && isHallCrusadesCampaignLockActive()) {
    hallCrusadesCampaignRecordPreparationTimer = window.setTimeout(prepare, 0);
    return;
  }

  hallCrusadesCampaignRecordPreparationTimer = window.setTimeout(() => {
    if (typeof window.requestAnimationFrame === "function") {
      hallCrusadesCampaignRecordPreparationFrame = window.requestAnimationFrame(prepare);
    } else {
      prepare();
    }
  }, delay);
...
function scheduleHallCrusadesCampaignRecordResolution() {
  ...
  hallCrusadesCampaignResolutionTimers = [];
  clearHallCrusadesCampaignRecordPreparationRequest();
  if (!isHallCrusadesCampaignRecordResolutionPrepared() && !renderHallCrusadesCampaignRecordResolution()) {
    return;
  }
...
function scheduleHallCrusadesCampaignCharge() {
  ...
  isHallCrusadesCampaignExpanding = false;
  isHallCrusadesCampaignExpanded = false;
  if (isHallCrusadesCampaignRecordResolutionPrepared()) {
    clearHallCrusadesCampaignRecordResolutionState();
  } else {
    clearHallCrusadesCampaignRecordResolution();
  }
...
function beginHallCrusadesCampaignLock(show, record) {
  ...
  if (!isHallCrusadesShowsVariantActive() || wrestlingShowsDataState !== "live" || !show) {
    return false;
  }

  const item = record?.closest?.(".hall-crusades-poster-strip__item");
  ...
  if (!isHallCrusadesCampaignRecordResolutionPrepared()) {
    queueHallCrusadesCampaignRecordPreparation(getHallCrusadesCampaignSelectedShow(), 180);
  }
  return true;
}
...
function syncHallCrusadesYearControls() {
  ...
  const isInteractionLocked = isHallCrusadesCampaignLockActive();
  const isDrawerVisible = isHallCrusadesShowsVariantActive() && isHallCrusadesYearDrawerOpen && !isInteractionLocked;

  if (hallCrusadesYearCrystal) {
    hallCrusadesYearCrystal.classList.toggle("is-active", isYearFiltered);
    hallCrusadesYearCrystal.disabled = isInteractionLocked;
    ...
    hallCrusadesYearCrystal.setAttribute("aria-disabled", String(isInteractionLocked));
...
function setHallCrusadesYearDrawerOpen(isOpen) {
  isHallCrusadesYearDrawerOpen = Boolean(isOpen && isHallCrusadesShowsVariantActive() && !isHallCrusadesCampaignLockActive());
...
function syncHallCrusadesBannerControls() {
  ...
  const isInteractionLocked = isHallCrusadesCampaignLockActive();
  const isDrawerVisible = isHallCrusadesShowsVariantActive() && isHallCrusadesBannerDrawerOpen && !isInteractionLocked;

  if (hallCrusadesBannerCrystal) {
    hallCrusadesBannerCrystal.classList.toggle("is-active", isBannerFiltered);
    hallCrusadesBannerCrystal.disabled = isInteractionLocked;
    ...
    hallCrusadesBannerCrystal.setAttribute("aria-disabled", String(isInteractionLocked));
...
function setHallCrusadesBannerDrawerOpen(isOpen) {
  isHallCrusadesBannerDrawerOpen = Boolean(isOpen && isHallCrusadesShowsVariantActive() && !isHallCrusadesCampaignLockActive());
...
function syncHallCrusadesFieldControls() {
  ...
  const isInteractionLocked = isHallCrusadesCampaignLockActive();
  const isDrawerVisible = isHallCrusadesShowsVariantActive() && isHallCrusadesFieldDrawerOpen && !isInteractionLocked;

  if (hallCrusadesFieldCrystal) {
    hallCrusadesFieldCrystal.classList.toggle("is-active", isFieldFiltered);
    hallCrusadesFieldCrystal.disabled = isInteractionLocked;
    ...
    hallCrusadesFieldCrystal.setAttribute("aria-disabled", String(isInteractionLocked));
...
function setHallCrusadesFieldDrawerOpen(isOpen) {
  isHallCrusadesFieldDrawerOpen = Boolean(isOpen && isHallCrusadesShowsVariantActive() && !isHallCrusadesCampaignLockActive());
...
function syncHallCrusadesSearchControls() {
  ...
  const isInteractionLocked = isHallCrusadesCampaignLockActive();
  const isPanelVisible = isHallCrusadesShowsVariantActive() && isHallCrusadesSearchPanelOpen && !isInteractionLocked;

  if (hallCrusadesSearchCrystal) {
    hallCrusadesSearchCrystal.classList.toggle("is-active", isSearchActive);
    hallCrusadesSearchCrystal.disabled = isInteractionLocked;
    ...
    hallCrusadesSearchCrystal.setAttribute("aria-disabled", String(isInteractionLocked));
  ...
  if (hallCrusadesSearchInput) {
    hallCrusadesSearchInput.disabled = isInteractionLocked;
    if (hallCrusadesSearchInput.value !== activeHallCrusadesSearchQuery) {
      hallCrusadesSearchInput.value = activeHallCrusadesSearchQuery;
    }
  }

  if (hallCrusadesSearchClear) {
    hallCrusadesSearchClear.disabled = isInteractionLocked || !isSearchActive;
  }
}
...
function focusHallCrusadesSearchInput() {
  ...
  window.requestAnimationFrame(() => {
    if (isHallCrusadesSearchPanelOpen && isHallCrusadesShowsVariantActive() && !isHallCrusadesCampaignLockActive()) {
...
function setHallCrusadesSearchPanelOpen(isOpen) {
  isHallCrusadesSearchPanelOpen = Boolean(isOpen && isHallCrusadesShowsVariantActive() && !isHallCrusadesCampaignLockActive());
...
function advanceHallCrusadesPosterActive(direction) {
  ...
  if (isHallCrusadesCampaignLockActive() || isDaiionCrusadesCarouselRevealLocked() || !isHallCrusadesShowsVariantActive() || total < 2) {
    return false;
  }
  ...
  const mobileIndexAdvance = !isHallCrusadesPosterDesktopKeyboard();
  const mobilePosterOpacities = mobileIndexAdvance ? new Map() : null;
  if (mobileIndexAdvance) {
    // Sample before changing flow direction so repeated moves keep their current brightness.
    hallCrusadesPosterStrip?.querySelectorAll(".hall-crusades-poster-strip__record").forEach((record) => {
      mobilePosterOpacities.set(record.parentElement.dataset.wrestlingShowId, window.getComputedStyle(record).opacity);
    });
  }
  ...
  renderHallCrusadesPosterStrip({ mobileIndexAdvance, mobilePosterOpacities });
  return true;
}
...
function syncHallCrusadesPosterNavControls(total = getHallCrusadesPosterSourceRows().length) {
  ...
  const isCampaignLocked = isHallCrusadesCampaignLockActive();
  const canOpenRecord = isHallCrusadesShowsVariantActive() && wrestlingShowsDataState === "live" && total > 0 && !isRevealLocked && !isCampaignLocked;
...
function handleHallCrusadesPosterKeyboardNavigation(event) {
  ...
  if (event.defaultPrevented || !isHallCrusadesShowsVariantActive()) {
    return;
  }
  ...
  if (!isHallCrusadesPosterDesktopKeyboard()) {
    return;
  }
...
function handleHallCrusadesPosterPointerDown(event) {
  if (isHallCrusadesCampaignLockActive() || !isHallCrusadesShowsVariantActive() || event.pointerType === "mouse") {
    return;
  }
...
function handleHallCrusadesPosterWheel(event) {
  ...
  if (!isHallCrusadesPosterDesktopWheel()) {
    return;
  }
...
function createHallCrusadesPosterStripEmptySlot(index = 0, options = {}) {
  ...
  if (options.isActive && isHallCrusadesCampaignLockActive() && hallCrusadesCampaignLockShowId === show.showId) {
    item.classList.add("is-campaign-lock-anchor");
  }
...
function renderHallCrusadesPosterStrip({ mobileIndexAdvance = false, mobilePosterOpacities = null } = {}) {
  if (!hallCrusadesPosterStrip) {
  ...
  posterRows.forEach((row, index) => {
    const item = row.isEmpty
      ? createHallCrusadesPosterStripEmptySlot(index, row)
      : createHallCrusadesPosterStripItem(row.show, index, row);
    const previousOpacity = mobilePosterOpacities?.get(item.dataset.wrestlingShowId);
    if (previousOpacity !== undefined) {
      item.querySelector(".hall-crusades-poster-strip__record").style.setProperty("--hall-crusades-flow-opacity", previousOpacity);
    }
  ...
  return {
    ...
    async reveal(root, targetShell, hero, animate) {
      ...
      const renderGallery = () => {
        ...
        if (controls) {
          controls.hidden = totalPages === 0;
          const indicator = controls.querySelector(".wrestling-match-dossier-photo-pagination__indicator");
          const label = `${pagination.pageNumber} / ${totalPages}`;
          if (indicator.textContent !== label) indicator.textContent = label;
          indicator.setAttribute("aria-label", `Page ${pagination.pageNumber} of ${totalPages}`);
          const buttons = controls.querySelectorAll("button");
          buttons[0].disabled = pageIndex <= 0;
          buttons[1].disabled = pageIndex >= totalPages - 1;
        }
        ...
        if (heading.parentElement !== gallery) gallery.append(heading);
        if (grid && grid.parentElement !== gallery) gallery.insertBefore(grid, status.parentElement === gallery ? status : null);
...
function createWrestlingMatchDossierPhotoPaginationButton(label, direction, pagination) {
  ...
  button.addEventListener("click", () => {
    if (!button.disabled) setWrestlingMatchDossierPhotoPage(pagination, pagination.pageIndex + direction);
  });
...
function createWrestlingMatchDossierPhotoPreviewGrid(pagination = null) {
  ...
  if (pagination.pagePhotos.length === 0) {
    pagination.reuseGrid?.remove();
    return null;
  }

  const grid = pagination.reuseGrid || document.createElement("div");
  const existingTiles = new Map([...grid.children].map((tile) => [tile.dataset.wrestlingSourcePhotoId, tile]));
  ...
  pagination.pagePhotos.forEach((photo, index) => {
    ...
    const existingTile = existingTiles.get(photo.photoId || "");
    const tile = existingTile || document.createElement(pagination.localOnly ? "div" : "button");
    ...
    if (!pagination.localOnly && !existingTile) tile.addEventListener("click", () => {
    ...
    if (image.parentElement !== tile) tile.append(image);
    retainedTiles.add(tile);
    if (grid.children[index] !== tile) grid.insertBefore(tile, grid.children[index] || null);
  });
  [...grid.children].forEach((tile) => { if (!retainedTiles.has(tile)) tile.remove(); });

  if (!pagination.reuseGrid) addWrestlingMatchDossierPhotoSwipeHandlers(grid, pagination);
...
function ownWrestlingMatchDossierPreparedRecord(pageKey, coordinator, hero, shell, animations, route) {
  ...
  const session = { pageKey, routeShowId, matchRef, coordinator, hero, shell, animations, observer };
...
function renderWrestlingMatchDetailPrototypeRoute(route = getRouteFromUrl(), options = {}) {
  ...
  const sameLiveEncounter = previous && previous.hero.parentElement === wrestlingMatchDetailPrototypeShell &&
    previous.routeShowId === getWrestlingMatchDetailRouteShowId(route) &&
    previous.matchRef === getWrestlingMatchDetailRouteMatchRef(route);
...
function getHallCrusadesCampaignPromotionDetailSurface() {
  if (!hallCrusadesCampaignRoutePromotion) {
    return null;
  }

  const detailHost = hallCrusadesCampaignRoutePromotion.detailHost?.isConnected
    ? hallCrusadesCampaignRoutePromotion.detailHost
    : getHallCrusadesCampaignExistingRecordDetailHost();
  const detailSurface = hallCrusadesCampaignRoutePromotion.detailSurface?.isConnected
    ? hallCrusadesCampaignRoutePromotion.detailSurface
    : detailHost?.querySelector?.(".wrestling-show-prototype-surface") || null;

  if (!detailHost || !detailSurface) {
    return null;
  }

  return { detailHost, detailSurface };
}

function getHallCrusadesCampaignPromotionRecordFrame(targetShell = wrestlingShowDetailShell) {
  if (!targetShell || targetShell !== wrestlingShowDetailShell || !hallCrusadesCampaignRoutePromotion) {
    return null;
  }

  const frame = hallCrusadesCampaignRoutePromotion.recordFrame?.isConnected
    ? hallCrusadesCampaignRoutePromotion.recordFrame
    : null;
  if (!frame) {
    return null;
  }
...
function promoteHallCrusadesCampaignResolvedRecordSurface(show, targetShell = wrestlingShowDetailShell, options = {}) {
  if (!targetShell || !hallCrusadesCampaignRoutePromotion || targetShell.dataset.hallCrusadesCampaignRecordDetail === "true") {
    return null;
  }

  const recordShowId = getHallCrusadesCampaignRecordShowId(show);
  if (!recordShowId || recordShowId !== hallCrusadesCampaignRoutePromotion.recordShowId) {
    return null;
  }

  const promotionSurface = getHallCrusadesCampaignPromotionDetailSurface();
  if (!promotionSurface) {
    return null;
  }
  ...
  applyHallCrusadesCampaignRecordScrollState(hallCrusadesCampaignRoutePromotion.scrollState);
...
function renderHallPrototypeShowDetailSurface(show, options = {}) {
  ...
  const promotedSurface = promoteHallCrusadesCampaignResolvedRecordSurface(show, targetShell, options);
  if (promotedSurface) {
    return promotedSurface;
  }
...
function renderWrestlingShowDetailRoute(showId = "warzone-26", options = {}) {
  ...
  const surface = renderHallPrototypeShowDetailSurface(show);
  completeHallCrusadesCampaignRoutePromotion(show, surface);
  return;
...
function getWrestlingPeoplePrototypeShell(options = {}) {
  ...
  pedestal.innerHTML = `
    <defs>
      <linearGradient id="hall-champions-stone-face" x1="0" y1="0" x2="0" y2="1">
        ...
        <stop offset="0.46" stop-color="#1b1412" />
        <stop offset="1" stop-color="#0d0908" />
      </linearGradient>
      <linearGradient id="hall-champions-stone-side" x1="0" y1="0" x2="1" y2="1">
        <stop offset="0" stop-color="#28211d" />
        <stop offset="0.56" stop-color="#15100f" />
        <stop offset="1" stop-color="#080606" />
      </linearGradient>
      <linearGradient id="hall-champions-metal-edge" x1="0" y1="0" x2="1" y2="0">
        <stop offset="0" stop-color="#3a3029" />
        <stop offset="0.5" stop-color="#76533e" />
        <stop offset="1" stop-color="#2a211d" />
      </linearGradient>
      <linearGradient id="hall-champions-channel-dormant" x1="0" y1="0" x2="1" y2="0">
        <stop offset="0" stop-color="#4a1a10" />
        <stop offset="0.52" stop-color="#a43d22" />
        <stop offset="1" stop-color="#45180e" />
      </linearGradient>
      <radialGradient id="hall-champions-crystal-recess" cx="50%" cy="44%" r="64%">
        <stop offset="0" stop-color="#30130c" />
    ...
    <g class="hall-of-champions-pedestal__base">
      <path d="M70 406 L570 406 L580 416 L60 416 Z" />
      <path d="M56 416 L584 416 L630 510 L10 510 Z" />
      <path d="M6 510 L634 510 L640 520 L0 520 Z" />
    </g>
  `;
  ...
  expandedHologram.innerHTML = `
    <section class="hall-of-champions-workspace hall-of-champions-workspace--search" data-hall-of-champions-workspace="search" aria-label="Hall of Champions search workspace" hidden>
      <h2 class="hall-of-champions-workspace__title">SEARCH ARCHIVE</h2>
      <label class="hall-of-champions-search-label" for="hall-of-champions-search">Name or alias</label>
      <input id="hall-of-champions-search" class="hall-of-champions-search-input" data-hall-of-champions-search type="search" autocomplete="off" placeholder="Search names and aliases">
      <p class="hall-of-champions-workspace__count" data-hall-of-champions-workspace-count="search">AWAITING QUERY</p>
      <div class="hall-of-champions-workspace__viewport" data-hall-of-champions-workspace-results="search" role="region" aria-label="Search archive records" aria-live="polite"></div>
    </section>
    <section class="hall-of-champions-workspace" data-hall-of-champions-workspace="az" aria-label="Hall of Champions alphabetical archive" hidden>
      <h2 class="hall-of-champions-workspace__title">ALPHABETICAL INDEX</h2>
      <p class="hall-of-champions-workspace__status" data-hall-of-champions-workspace-status="az">CURRENT INITIAL // A</p>
      <p class="hall-of-champions-workspace__count" data-hall-of-champions-workspace-count="az">INDEXING RECORDS</p>
      <div class="hall-of-champions-workspace__viewport" data-hall-of-champions-workspace-results="az" role="region" aria-label="Alphabetical archive records" aria-live="polite"></div>
    </section>
    <section class="hall-of-champions-workspace" data-hall-of-champions-workspace="category" aria-label="Hall of Champions category archive" hidden>
      <h2 class="hall-of-champions-workspace__title">ARCHIVE CATEGORIES</h2>
      <p class="hall-of-champions-workspace__status" data-hall-of-champions-workspace-status="category">CURRENT CATEGORY // DISCOVERING</p>
      <p class="hall-of-champions-workspace__count" data-hall-of-champions-workspace-count="category">INDEXING RECORDS</p>
      <div class="hall-of-champions-workspace__viewport" data-hall-of-champions-workspace-results="category" role="region" aria-label="Category archive records" aria-live="polite"></div>
    </section>
    <section class="hall-of-champions-workspace" data-hall-of-champions-workspace="team" aria-label="Hall of Champions team archive" hidden>
      <h2 class="hall-of-champions-workspace__title">TEAM DIRECTORY</h2>
      <p class="hall-of-champions-workspace__status" data-hall-of-champions-workspace-status="team">CURRENT TEAM // DISCOVERING</p>
      <p class="hall-of-champions-workspace__count" data-hall-of-champions-workspace-count="team">INDEXING RECORDS</p>
      <div class="hall-of-champions-workspace__viewport" data-hall-of-champions-workspace-results="team" role="region" aria-label="Team archive records" aria-live="polite"></div>
    </section>
  `;

  expandedHologram.querySelector("[data-hall-of-champions-search]").addEventListener("input", (event) => {
    hallOfChampionsSearchQuery = event.currentTarget.value;
    renderHallOfChampionsArchiveRows(prototypeShell, "search");
  });
  ...
  modeSelector.dataset.hallOfChampionsModeSelector = "true";
  ...
  modeSelector.setAttribute("aria-label", "Hall of Champions archive mode selector. Active mode: SEARCH");
  modeSelector.setAttribute("aria-hidden", "true");
  modeSelector.hidden = true;
  modeSelector.innerHTML = `
    <button class="hall-of-champions-mode-selector__control hall-of-champions-mode-selector__control--previous" type="button" data-hall-of-champions-mode-nav="previous" aria-label="Previous archive mode" aria-disabled="true" disabled>
      <span aria-hidden="true">&lsaquo;</span>
    </button>
    <div class="hall-of-champions-mode-selector__readout" aria-live="polite" aria-atomic="true">
      <span class="hall-of-champions-mode-selector__label hall-of-champions-mode-selector__label--current" data-hall-of-champions-mode-label="current">SEARCH</span>
      <span class="hall-of-champions-mode-selector__label hall-of-champions-mode-selector__label--incoming" data-hall-of-champions-mode-label="incoming" aria-hidden="true"></span>
    </div>
    <button class="hall-of-champions-mode-selector__control hall-of-champions-mode-selector__control--next" type="button" data-hall-of-champions-mode-nav="next" aria-label="Next archive mode" aria-disabled="true" disabled>
      <span aria-hidden="true">&rsaquo;</span>
    </button>
  `;
  modeSelector.querySelectorAll("[data-hall-of-champions-mode-nav]").forEach((button) => {
    button.addEventListener("click", handleHallOfChampionsModeNavigation);
  });
...
function syncHallOfChampionsProjectionGeometry(prototypeShell) {
  ...
  const crystal = prototypeShell.querySelector(".hall-of-champions-pedestal__crystal");
  const pedestal = prototypeShell.querySelector(".hall-of-champions-pedestal");
  ...
  if (pedestal) {
    ...
    if (pedestalRect.width > 0 && pedestalRect.height > 0) {
      prototypeShell.style.setProperty("--hall-champions-mode-selector-left", `${pedestalRect.left + (pedestalRect.width * (202 / 640))}px`);
      prototypeShell.style.setProperty("--hall-champions-mode-selector-top", `${pedestalRect.top + (pedestalRect.height * (292 / 520))}px`);
      prototypeShell.style.setProperty("--hall-champions-mode-selector-width", `${pedestalRect.width * (236 / 640)}px`);
      prototypeShell.style.setProperty("--hall-champions-mode-selector-height", `${pedestalRect.height * (112 / 520)}px`);
      prototypeShell.style.setProperty("--hall-champions-mode-readout-left", pedestalRect.width * (58 / 640) + "px");
      prototypeShell.style.setProperty("--hall-champions-mode-readout-top", pedestalRect.height * (22 / 520) + "px");
      prototypeShell.style.setProperty("--hall-champions-mode-readout-width", pedestalRect.width * (120 / 640) + "px");
      prototypeShell.style.setProperty("--hall-champions-mode-readout-height", pedestalRect.height * (88 / 520) + "px");
      prototypeShell.style.setProperty("--hall-champions-mode-arrow-side", pedestalRect.width * (8 / 640) + "px");
      prototypeShell.style.setProperty("--hall-champions-mode-arrow-top", pedestalRect.height * (30 / 520) + "px");
      prototypeShell.style.setProperty("--hall-champions-mode-arrow-width", pedestalRect.width * (44 / 640) + "px");
      prototypeShell.style.setProperty("--hall-champions-mode-arrow-height", pedestalRect.height * (78 / 520) + "px");
      const baseFaces = pedestal.querySelectorAll(".hall-of-champions-pedestal__base path");
      const face = baseFaces[1];
      ...
      if (matrix && baseFaces.length >= 3) {
        ...
        const controlsBottom = points[1] + 26;
        const labelTop = points[1] + 28;
        const labelBottom = points[5] - 2;
        ...
        prototypeShell.style.setProperty("--hall-champions-letter-label-width", (points[4] - points[6] - 28) * scaleX + "px");
...
function getHallOfChampionsModeSelector(prototypeShell) {
...
function resetHallOfChampionsModeSelector(prototypeShell) {
  ...
  if (currentLabel) {
    currentLabel.textContent = HALL_OF_CHAMPIONS_ARCHIVE_MODES[0];
  }
  if (incomingLabel) {
...
function completeHallOfChampionsModeTransition(modeSelector, nextIndex) {
  const currentLabel = modeSelector?.querySelector("[data-hall-of-champions-mode-label='current']");
  const incomingLabel = modeSelector?.querySelector("[data-hall-of-champions-mode-label='incoming']");
  const nextMode = HALL_OF_CHAMPIONS_ARCHIVE_MODES[nextIndex] || HALL_OF_CHAMPIONS_ARCHIVE_MODES[0];

  if (currentLabel) {
    currentLabel.textContent = nextMode;
  }
  if (incomingLabel) {
    incomingLabel.textContent = "";
    incomingLabel.setAttribute("aria-hidden", "true");
...
function cycleHallOfChampionsArchiveMode(prototypeShell, direction) {
  const modeSelector = getHallOfChampionsModeSelector(prototypeShell);
  if (!modeSelector || prototypeShell?.dataset.hallOfChampionsModeSelectorReady !== "true") {
    return;
  }
  if (modeSelector.dataset.hallOfChampionsModeStatus === "changing") {
    return;
  }

  const currentIndex = Number.parseInt(modeSelector.dataset.hallOfChampionsModeIndex || "0", 10);
  ...
  modeSelector.dataset.hallOfChampionsModeDirection = direction === "previous" ? "previous" : "next";
  if (hallOfChampionsModeTransitionTimer) {
    window.clearTimeout(hallOfChampionsModeTransitionTimer);
  }
  hallOfChampionsModeTransitionTimer = window.setTimeout(() => {
    hallOfChampionsModeTransitionTimer = 0;
    completeHallOfChampionsModeTransition(modeSelector, nextIndex);
  }, HALL_OF_CHAMPIONS_MODE_TRANSITION_MS);
}

function handleHallOfChampionsModeNavigation(event) {
  event.preventDefault();
  const prototypeShell = getHallOfChampionsPrototypeShellFromNode(event.currentTarget);
  const direction = event.currentTarget?.dataset.hallOfChampionsModeNav === "previous" ? "previous" : "next";
...
function handleHallOfChampionsModeSelectorKeydown(event) {
  ...
  if (event.key === "ArrowLeft") {
    event.preventDefault();
    cycleHallOfChampionsArchiveMode(prototypeShell, "previous");
    return;
  }
  if (event.key === "ArrowRight") {
    event.preventDefault();
    cycleHallOfChampionsArchiveMode(prototypeShell, "next");
  }
}

function scheduleHallOfChampionsModeSelectorAvailability(prototypeShell) {
  if (!prototypeShell) {
    return;
  }
  ...
  const revealDelay = prefersHallOfChampionsReducedMotion()
    ? 0
    : HALL_OF_CHAMPIONS_MODE_TRANSITION_MS;

  hallOfChampionsModeSelectorRevealTimer = window.setTimeout(() => {
    hallOfChampionsModeSelectorRevealTimer = 0;
    if (
      prototypeShell.dataset.hallOfChampionsActive === "true"
...
function getHallOfChampionsPeopleArchiveDisplayName(record) {
...
function getHallOfChampionsPeopleArchivePhotoCount(record) {
  const fields = ["photoCount", "photo_count", "photosCount", "photos_count", "totalPhotos", "total_photos", "imageCount", "image_count", "mediaCount", "media_count", "photos"];
  for (const field of fields) {
  ...
  return (Array.isArray(payloads) ? payloads : [payloads]).flatMap(getHallOfChampionsPeopleArchiveRows).map((record, index) => {
    try {
      ...
      return key && name ? {
        ...
        name,
        initial,
        aliases: getWrestlingPersonAliases(record),
...
function getHallOfChampionsPeopleArchiveStorage() {
  try {
...
function hydrateHallOfChampionsPeopleArchiveFromCache(prototypeShell) {
  if (hallOfChampionsPeopleArchiveCacheHydrated) {
    return false;
  }
  hallOfChampionsPeopleArchiveCacheHydrated = true;
  const storage = getHallOfChampionsPeopleArchiveStorage();
  if (!storage) {
    return false;
  }
  try {
    const cached = JSON.parse(storage.getItem(HALL_OF_CHAMPIONS_PEOPLE_ARCHIVE_CACHE_KEY) || "null");
    const cachedAt = Number.parseInt(cached?.cachedAt, 10);
    if (!cached || cached.version !== 1 || !Array.isArray(cached.records) || !Number.isFinite(cachedAt)) {
      return false;
    }
    if (Date.now() - cachedAt > HALL_OF_CHAMPIONS_PEOPLE_ARCHIVE_CACHE_MAX_AGE_MS) {
      storage.removeItem(HALL_OF_CHAMPIONS_PEOPLE_ARCHIVE_CACHE_KEY);
      return false;
    }
    const records = normalizeHallOfChampionsPeopleArchiveCachedRecords(cached.records);
    if (!records.length) {
      return false;
    }
    hallOfChampionsPeopleArchiveRecords = records;
    hallOfChampionsPeopleArchiveState = "live";
    hallOfChampionsPeopleArchiveNetworkComplete = false;
    syncHallOfChampionsDiscoveredSelectorValues(prototypeShell);
    syncHallOfChampionsWorkspaces(prototypeShell);
    return true;
  } catch (error) {
    reportHallOfChampionsPeopleArchiveRecordIssue("cached archive unavailable", error, null);
    return false;
  }
}

function writeHallOfChampionsPeopleArchiveCache(records) {
  const storage = getHallOfChampionsPeopleArchiveStorage();
  if (!storage || !Array.isArray(records) || !records.length) {
    return;
  }
  try {
    ...
    reportHallOfChampionsPeopleArchiveRecordIssue("cached archive write skipped", error, null);
  }
}
...
function requestHallOfChampionsPeopleSelectorData(prototypeShell) {
  ...
  hydrateHallOfChampionsPeopleArchiveFromCache(prototypeShell);
  if (hallOfChampionsPeopleArchiveRecords.length > 0 && hallOfChampionsPeopleArchiveNetworkComplete) {
    return Promise.resolve(hallOfChampionsPeopleArchiveRecords);
  }
  if (hallOfChampionsPeopleArchiveRequest || typeof fetch !== "function") {
    return hallOfChampionsPeopleArchiveRequest || Promise.resolve(hallOfChampionsPeopleArchiveRecords);
  }
  const hasCachedRecords = hallOfChampionsPeopleArchiveRecords.length > 0;
  hallOfChampionsPeopleArchiveState = hasCachedRecords ? "live" : "loading";
  syncHallOfChampionsWorkspaces(prototypeShell);
  const controller = typeof AbortController === "function" ? new AbortController() : null;
  ...
  hallOfChampionsPeopleArchiveRequest = fetchHallOfChampionsPeopleArchiveSelectorPayload(1, signal).then((firstPayload) => {
    ...
    hallOfChampionsPeopleArchiveState = hallOfChampionsPeopleArchiveRecords.length > 0 ? "live" : "empty";
    hallOfChampionsPeopleArchiveNetworkComplete = true;
    syncHallOfChampionsDiscoveredSelectorValues(prototypeShell);
    syncHallOfChampionsWorkspaces(prototypeShell);
    writeHallOfChampionsPeopleArchiveCache(hallOfChampionsPeopleArchiveRecords);
...
function getHallOfChampionsTeamFilterKey(value) {
  return normalizeHallOfChampionsPeopleArchiveDiscoveredTeam(value).toLowerCase();
}

function getHallOfChampionsPeopleArchiveRecordTeams(record) {
  return normalizeHallOfChampionsPeopleArchiveDiscoveredTeam(record?.team).split(/[;,]/).map(normalizeHallOfChampionsPeopleArchiveDiscoveredTeam).filter(Boolean);
}

function getHallOfChampionsActiveWorkspaceMode(prototypeShell) {
  const activeModeIndex = getHallOfChampionsCurrentModeIndex(prototypeShell);
  return activeModeIndex === 1 ? "az" : activeModeIndex === 2 ? "category" : activeModeIndex === 3 ? "team" : "search";
}

let hallOfChampionsSearchQuery = "";
const hallOfChampionsReturnSnapshots = new Map();
let hallOfChampionsReturnSequence = 0;
...
function restoreHallOfChampionsReturn(prototypeShell) {
  ...
  hallOfChampionsSearchQuery = matching ? snapshot.query : "";
  prototypeShell.querySelector("[data-hall-of-champions-search]").value = hallOfChampionsSearchQuery;
  hallOfChampionsAzLetterIndex = matching ? Math.max(0, HALL_OF_CHAMPIONS_AZ_LETTERS.indexOf(snapshot.letter)) : 0;
  hallOfChampionsCategoryIndex = matching ? Math.max(0, hallOfChampionsPeopleArchiveCategories.indexOf(snapshot.category)) : 0;
  hallOfChampionsTeamIndex = matching ? Math.max(0, hallOfChampionsPeopleArchiveTeams.indexOf(snapshot.team)) : 0;
  prototypeShell.dataset.hallOfChampionsExpanded = String(Boolean(matching && snapshot.expanded));
...
function restoreHallOfChampionsReturnFocus(prototypeShell, viewport) {
...
function getHallOfChampionsActiveWorkspaceMatches(modeName) {
  if (modeName === "search") {
    const query = hallOfChampionsSearchQuery.trim().toLowerCase();
    return query ? hallOfChampionsPeopleArchiveRecords.filter((record) =>
      [record.name, ...(record.aliases || [])].some((name) => String(name).toLowerCase().includes(query))) : [];
  }
  if (modeName === "az") {
  ...
  if (modeName === "category") {
    ...
    return selectedCategoryKey
      ? hallOfChampionsPeopleArchiveRecords.filter((record) => getHallOfChampionsCategoryFilterKey(record?.category) === selectedCategoryKey)
      : [];
  }
  if (modeName === "team") {
    const selectedTeamKey = getHallOfChampionsTeamFilterKey(getHallOfChampionsCurrentTeam());
    return selectedTeamKey
      ? hallOfChampionsPeopleArchiveRecords.filter((record) => getHallOfChampionsPeopleArchiveRecordTeams(record).some((teamName) => getHallOfChampionsTeamFilterKey(teamName) === selectedTeamKey))
      : [];
  }
  return [];
}
...
function appendHallOfChampionsArchiveRecord(list, record) {
  try {
    ...
    item.className = "hall-of-champions-workspace__record";
    marker.className = "hall-of-champions-workspace__record-light";
    marker.setAttribute("aria-hidden", "true");
    primary.className = "hall-of-champions-workspace__record-name";
    primary.textContent = getWrestlingText(record?.name).trim();
    secondary.className = "hall-of-champions-workspace__record-team";
    secondary.textContent = getWrestlingText(record?.team).trim();
    if (!secondary.textContent) {
      secondary.hidden = true;
    }
    category.className = "hall-of-champions-workspace__record-category";
    category.textContent = getWrestlingText(record?.category).trim();
    if (!category.textContent) {
      category.hidden = true;
    }
    photos.className = "hall-of-champions-workspace__record-photos";
    const photoText = formatHallOfChampionsArchiveRecordPhotoCount(record?.photoCount);
    photos.textContent = photoText;
...
function renderHallOfChampionsArchiveRows(prototypeShell, modeName) {
  ...
  const count = prototypeShell?.querySelector(`[data-hall-of-champions-workspace-count="${modeName}"]`);
  const viewport = prototypeShell?.querySelector(`[data-hall-of-champions-workspace-results="${modeName}"]`);
  if (!workspace || !viewport || !count) {
    return;
  }
  ...
  if (modeName === "search" && !hallOfChampionsSearchQuery.trim()) {
    count.textContent = "AWAITING QUERY";
    const standby = document.createElement("p");
    standby.className = "hall-of-champions-workspace__placeholder";
    standby.textContent = "SEARCH WORKSPACE STANDBY";
    viewport.append(standby);
    return;
  }
  workspace.dataset.hallOfChampionsWorkspaceState = hallOfChampionsPeopleArchiveState;

  if (modeName === "az" && status) {
    status.textContent = `CURRENT INITIAL // ${getHallOfChampionsCurrentAzLetter()}`;
  } else if (modeName === "category" && status) {
    status.textContent = getHallOfChampionsCurrentCategory() ? `CURRENT CATEGORY // ${getHallOfChampionsCurrentCategory()}` : "CURRENT CATEGORY // DISCOVERING";
  } else if (modeName === "team" && status) {
    status.textContent = getHallOfChampionsCurrentTeam() ? `CURRENT TEAM // ${getHallOfChampionsCurrentTeam()}` : "CURRENT TEAM // DISCOVERING";
  }

  if (hallOfChampionsPeopleArchiveState === "loading" && matches.length === 0) {
    count.textContent = "INDEXING RECORDS";
    const message = document.createElement("p");
    message.className = "hall-of-champions-workspace__message";
  ...
  if (hallOfChampionsPeopleArchiveState === "error" && matches.length === 0) {
    count.textContent = "ARCHIVE LINK UNAVAILABLE";
    const message = document.createElement("p");
    message.className = "hall-of-champions-workspace__message";
    message.textContent = "ARCHIVE LINK UNAVAILABLE";
    message.setAttribute("role", "status");
    viewport.append(message);
    return;
  }

  count.textContent = `${matches.length.toLocaleString()} RECORDS`;
  if (matches.length === 0) {
    const message = document.createElement("p");
    message.className = "hall-of-champions-workspace__message";
    message.textContent = modeName === "search" ? "NO MATCHING RECORDS" : modeName === "az" ? `NO RECORDS FOUND FOR // ${getHallOfChampionsCurrentAzLetter()}` : "NO ARCHIVE RECORDS";
    message.setAttribute("role", "status");
    viewport.append(message);
    return;
  }
  ...
  const usesPortraitCards = modeName === "search" || modeName === "az" || modeName === "category" || modeName === "team";
  ...
  viewport.append(list);
  if (focusedPerson) {
    [...viewport.querySelectorAll("[data-wrestling-person-id]")].find((node) => node.dataset.wrestlingPersonId === focusedPerson)?.focus({ preventScroll: true });
    viewport.scrollTop = previousScroll;
  }
  restoreHallOfChampionsReturnFocus(prototypeShell, viewport);
}

function syncHallOfChampionsWorkspaces(prototypeShell) {
  if (!prototypeShell) {
    return;
  }
...
function startHallOfChampionsPedestalActivation(prototypeShell) {
  ...
  if (prefersHallOfChampionsReducedMotion()) {
    ...
    if (prototypeShell.dataset.hallOfChampionsExpanded === "true") {
      scheduleHallOfChampionsModeSelectorAvailability(prototypeShell);
    }
    return;
  }
...
function getWrestlingPersonDossierPrototypeUniqueTextValues(values) {
  return (Array.isArray(values) ? values : [])
    .map((value) => getWrestlingPersonDossierPrototypeText(value))
    .filter((value, index, list) => value && list.findIndex((candidate) => candidate.toLowerCase() === value.toLowerCase()) === index);
}

function getWrestlingPersonDossierPrototypeRecordDisplayName(record, fallback) {
  return getWrestlingPersonDossierPrototypeDisplayValue(
    record?.name || record?.display_name || record?.displayName || record?.ring_name || record?.ringName || record?.title,
    fallback
  );
}

function getWrestlingPersonDossierPrototypeRecordStableId(record, fallback = "") {
  const source = record?.backend_record && typeof record.backend_record === "object"
    ? { ...record.backend_record, ...record }
    : record;
  return [
    ...
    source?.id,
    source?.slug,
    source ? getWrestlingPersonRouteId(source) : "",
    fallback,
  ]
    .map((value) => normalizeWrestlingPersonId(value))
    .find(Boolean) || "";
}

function getWrestlingPersonDossierPrototypeSelectedPersonContext(record = wrestlingPersonDossierPrototypeSelectedPersonState.record) {
  const seed = wrestlingPersonDossierPrototypeSelectedPersonState.seed || {};
  const source = record?.backend_record && typeof record.backend_record === "object"
    ? { ...record.backend_record, ...record }
    : record;
  const seedName = getWrestlingPersonDossierPrototypeDisplayValue(seed.displayName || seed.name || seed.searchName, "Archive Record");
  const displayName = getWrestlingPersonDossierPrototypeRecordDisplayName(source, seedName);
  const participantName = getWrestlingPersonDossierPrototypeDisplayValue(source?.participant_name || source?.participantName || seed.participantName || displayName, displayName);
  const stableId = getWrestlingPersonDossierPrototypeRecordStableId(source, seed.slug || seed.personId || displayName);
  const aliases = getWrestlingPersonDossierPrototypeUniqueTextValues([
    ...getWrestlingPersonAliases(source),
    ...getWrestlingPersonAliases(record),
  ]);
  const identityMatchNames = getWrestlingPersonDossierPrototypeUniqueTextValues([
    displayName,
    participantName,
    source?.name,
    source?.display_name,
  ...
  return {
    ...
    record: source || null,
    displayName,
    normalizedDisplayName: normalizeWrestlingPersonDossierPrototypeName(displayName),
    stableId,
    slug: stableId,
    personId: stableId,
    category: source ? normalizeWrestlingPersonDossierPrototypeCategory(source?.category, WRESTLING_PERSON_DOSSIER_PROTOTYPE_METADATA_FALLBACK) : WRESTLING_PERSON_DOSSIER_PROTOTYPE_METADATA_FALLBACK,
    aliases,
    teamStable: source ? getWrestlingPersonDossierPrototypeTeamStable(source) : WRESTLING_PERSON_DOSSIER_PROTOTYPE_METADATA_FALLBACK,
    portraitUrl: getWrestlingPersonDossierPrototypePortraitUrl(source),
    participantName,
    searchName: getWrestlingPersonDossierPrototypeDisplayValue(seed.searchName || participantName || displayName, displayName),
    identityMatchNames,
    normalizedIdentityMatchNames: identityMatchNames.map((value) => normalizeWrestlingPersonDossierPrototypeName(value)).filter(Boolean),
    captionMatchNames: identityMatchNames,
  };
}

function getWrestlingPersonDossierPrototypePersonLookupUrl(context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const apiUrl = new URL(WRESTLING_PEOPLE_API_ROUTE, WRESTLING_SHOWS_API_BASE_URL);
  apiUrl.searchParams.set("search", context.searchName || context.displayName);
  apiUrl.searchParams.set("limit", "25");
  apiUrl.searchParams.set("page", "1");
  return apiUrl.href;
}

function getWrestlingPersonDossierPrototypeEventHistoryUrl(context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const apiUrl = new URL(WRESTLING_SHOWS_API_ROUTE, WRESTLING_SHOWS_API_BASE_URL);
  apiUrl.searchParams.set("participant", context.participantName || context.displayName);
  apiUrl.searchParams.set("sort", "date");
  apiUrl.searchParams.set("dir", "desc");
...
function resetWrestlingPersonDossierPrototypeEventHistoryPersonState() {
  ...
  state.diagnosticsLogKey = "";
  state.error = null;
  state.requestPromise = null;
  state.requestKey = "";
}

function resetWrestlingPersonDossierPrototypeEventArchivePersonState() {
  ...
  state.photoRenderItems = [];
  state.photoGalleryPage = 1;
  state.mode = "grid";
  ...
  if (region) {
    ...
    delete region.dataset.wrestlingPersonDossierPrototypeEventArchivePhotoState;
    region.replaceChildren();
  }
}

function resetWrestlingPersonDossierPrototypeSelectedPersonData() {
  ...
  resetWrestlingPersonDossierPrototypeEventArchivePersonState();
}

function setWrestlingPersonDossierPrototypeSelectedSubject(subject = {}) {
  const state = wrestlingPersonDossierPrototypeSelectedPersonState;
  const seed = { ...(subject || {}) };
  ...
  state.seed = seed;
  state.subjectKey = nextKey;
  resetWrestlingPersonDossierPrototypeSelectedPersonData();
  return getWrestlingPersonDossierPrototypeSelectedPersonContext();
}

function getWrestlingPersonDossierPrototypeRecords(payload) {
  if (!payload || !Array.isArray(payload.data)) {
    throw new Error("Unexpected prototype person response structure");
  }
  return payload.data;
}
...
function findWrestlingPersonDossierPrototypeSelectedPersonRecord(payload, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  ...
  const idMatch = records.find((record) => [
    record?.personId,
    record?.wrestling_person_id,
    record?.person_id,
    record?.id,
    record?.slug,
    getWrestlingPersonRouteId(record),
  ].some((value) => targetIds.has(normalizeWrestlingPersonId(value))));
  if (idMatch) {
    return idMatch;
  }

  const targetNames = new Set([context.displayName, context.participantName, context.searchName, ...context.identityMatchNames]
    .map((value) => normalizeWrestlingPersonDossierPrototypeName(value))
    .filter(Boolean));
  return records.find((record) => [
    ...
    record?.ring_name,
    record?.ringName,
  ].some((value) => targetNames.has(normalizeWrestlingPersonDossierPrototypeName(value)))) || null;
}
function getWrestlingPersonDossierPrototypeReadoutName(context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  return context.displayName;
}

function getWrestlingPersonDossierPrototypeReadoutText(context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  return `Champion Dossier - ${getWrestlingPersonDossierPrototypeReadoutName(context)}`;
}
...
function renderWrestlingPersonDossierPrototypePortrait(workspace, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
...
function getWrestlingPersonDossierPrototypeEventHistoryRows(payload) {
  if (!payload || !Array.isArray(payload.data)) {
    throw new Error("Unexpected prototype event-history response structure");
  }
  return payload.data;
}
...
function isWrestlingPersonDossierPrototypeSelectedPersonIdentity(value, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const normalizedValue = normalizeWrestlingPersonDossierPrototypeName(value);
  return Boolean(normalizedValue && context.normalizedIdentityMatchNames.includes(normalizedValue));
}
...
function hasWrestlingPersonDossierPrototypeSelectedPersonInMatch(match, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  return getWrestlingPersonDossierPrototypeMatchPeople(match, ["participants", "participant_names", "participantNames", "side_1", "side_2", "winner"])
    .some((value) => isWrestlingPersonDossierPrototypeSelectedPersonIdentity(value, context));
}

function getWrestlingPersonDossierPrototypeSelectedPersonSideKey(match, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  ...
  if (side1.some((value) => isWrestlingPersonDossierPrototypeSelectedPersonIdentity(value, context))) {
    return "side_1";
  }
  if (side2.some((value) => isWrestlingPersonDossierPrototypeSelectedPersonIdentity(value, context))) {
    return "side_2";
  }
  return "";
}
...
function getWrestlingPersonDossierPrototypeAppearanceResult(match, personSideKey, personSideValues, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const winnerValues = getWrestlingPersonDossierPrototypeEventHistoryTextList(match?.winner);
  if (winnerValues.some((value) => isWrestlingPersonDossierPrototypeSelectedPersonIdentity(value, context))) {
    return "Winner";
  }
  if (personSideKey && winnerValues.some((winner) => personSideValues.some((sideValue) => normalizeWrestlingPersonDossierPrototypeName(sideValue) === normalizeWrestlingPersonDossierPrototypeName(winner)))) {
    return "Winner";
  }
  return hasWrestlingPersonDossierPrototypeSelectedPersonInMatch(match, context) ? "Participant" : "Result Unlisted";
}
...
function hasWrestlingPersonDossierPrototypeSelectedPersonInWinParticipantMatch(match, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
...
function isWrestlingPersonDossierPrototypeExplicitMatchWin(match, personSideKey, personSideValues, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  ...
  const personSideLabel = formatWrestlingPersonDossierPrototypeEventHistoryList(personSideValues, context.displayName);
  return winnerValues.some((winner) => {
    const normalizedWinner = normalizeWrestlingPersonDossierPrototypeName(winner);
    return personSideValues.some((sideValue) => normalizeWrestlingPersonDossierPrototypeName(sideValue) === normalizedWinner) || normalizeWrestlingPersonDossierPrototypeName(personSideLabel) === normalizedWinner;
  });
}
...
function getWrestlingPersonDossierPrototypeWinParticipantRecord(events, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  ...
  events.forEach((event) => {
    ...
    appearances.forEach((appearance, appearanceIndex) => {
      const match = appearance?.match;
      if (!match || typeof match !== "object" || isWrestlingPersonDossierPrototypeNonMatchSegment(match) || !hasWrestlingPersonDossierPrototypeSelectedPersonInWinParticipantMatch(match, context)) {
        return;
      }
      ...
      const personSideKey = getWrestlingPersonDossierPrototypeSelectedPersonSideKey(match, context);
      const personSideValues = personSideKey ? getWrestlingPersonDossierPrototypeMatchSide(match, personSideKey) : [context.displayName];
      if (isWrestlingPersonDossierPrototypeExplicitMatchWin(match, personSideKey, personSideValues, context)) {
        record.wins += 1;
      } else {
...
function setWrestlingPersonDossierPrototypeWinParticipantStatus(scope, events, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const record = getWrestlingPersonDossierPrototypeWinParticipantRecord(Array.isArray(events) ? events : [], context);
  setWrestlingPersonDossierPrototypeStatusItem(scope, "winParticipant", record.wins + " / " + record.participant);
}

function getWrestlingPersonDossierPrototypePeopleMatchTotal(record) {
  const value = [record?.match_count, record?.matchCount, record?.matches_count]
    .map((candidate) => getWrestlingPersonDossierPrototypeText(candidate))
    .find((candidate) => candidate && Number.isFinite(Number(candidate)));
  return value ? Number(value) : null;
}

function getWrestlingPersonDossierPrototypeRawMatchOrder(match, matchIndex) {
  const matchOrder = Number(match?.match_order ?? match?.matchOrder ?? match?.order ?? matchIndex + 1);
  return Number.isFinite(matchOrder) ? matchOrder : matchIndex + 1;
}

function getWrestlingPersonDossierPrototypeRawMatchRecordKey(event, match, matchIndex) {
  return getWrestlingPersonDossierPrototypeMatchRecordKey(event, {
    match,
    matchOrder: getWrestlingPersonDossierPrototypeRawMatchOrder(match, matchIndex),
  }, matchIndex);
}

function getWrestlingPersonDossierPrototypeExplicitWinType(match, personSideKey, personSideValues, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const winnerValues = getWrestlingPersonDossierPrototypeMatchWinnerValues(match);
  if (winnerValues.some((value) => isWrestlingPersonDossierPrototypeSelectedPersonIdentity(value, context))) {
    return "directPerson";
  }
  ...
  const personSideLabel = formatWrestlingPersonDossierPrototypeEventHistoryList(personSideValues, context.displayName);
  const hasPersonSideWinner = winnerValues.some((winner) => {
    const normalizedWinner = normalizeWrestlingPersonDossierPrototypeName(winner);
    return personSideValues.some((sideValue) => normalizeWrestlingPersonDossierPrototypeName(sideValue) === normalizedWinner) || normalizeWrestlingPersonDossierPrototypeName(personSideLabel) === normalizedWinner;
  });
  return hasPersonSideWinner ? "personSideTeam" : "";
}

function getWrestlingPersonDossierPrototypeParticipantDiagnosticType(match, personSideKey) {
  const winnerValues = getWrestlingPersonDossierPrototypeMatchWinnerValues(match);
  if (winnerValues.length > 0) {
    return "explicitNonWinning";
  }
  return personSideKey ? "missingUnknownWinner" : "otherStructuredNonWinning";
}

function getWrestlingPersonDossierPrototypeWinParticipantDiagnostics(events, peopleRecord, context = getWrestlingPersonDossierPrototypeSelectedPersonContext(peopleRecord)) {
  const diagnostics = {
    totalTimelineEvents: events.length,
    eventsWithEmbeddedMatchData: 0,
    eventsWithoutEmbeddedMatchData: 0,
    rawEmbeddedMatchRecords: 0,
    dedupedEmbeddedMatchRecords: 0,
    qualifyingPersonMatches: 0,
    duplicateMatchesRemoved: 0,
    explicitDirectPersonWins: 0,
    explicitPersonSideTeamWins: 0,
    ambiguousWinnerTextWins: 0,
    participant: {
      total: 0,
      explicitNonWinning: 0,
      missingUnknownWinner: 0,
      otherStructuredNonWinning: 0,
    },
    nonMatchSegmentExclusions: 0,
    ambiguousUnclassifiableRecords: 0,
    eventBuckets: {
      personQualifyingMatchFound: 0,
      multiplePersonQualifyingMatchesFound: 0,
      embeddedMatchesNoQualifyingPersonMatch: 0,
      noEmbeddedMatchData: 0,
      nonMatchPersonSegmentOnly: 0,
      ambiguousIncompleteMatchStructure: 0,
    },
    peopleMatches: getWrestlingPersonDossierPrototypePeopleMatchTotal(peopleRecord),
    visibleRecord: getWrestlingPersonDossierPrototypeWinParticipantRecord(events, context),
    unreconciledDifference: null,
  };
  const seenEmbeddedMatches = new Set();
  const seenQualifyingMatches = new Set();

  events.forEach((event) => {
    const rawMatches = Array.isArray(event?.raw?.matches) ? event.raw.matches : [];
    const appearances = Array.isArray(event?.personAppearances) ? event.personAppearances : [];
    let eventQualifyingMatches = 0;
    let eventNonMatchPersonSegments = 0;
    let eventAmbiguousRecords = 0;

    if (rawMatches.length === 0) {
      diagnostics.eventsWithoutEmbeddedMatchData += 1;
      diagnostics.eventBuckets.noEmbeddedMatchData += 1;
      return;
    }

    diagnostics.eventsWithEmbeddedMatchData += 1;
    diagnostics.rawEmbeddedMatchRecords += rawMatches.length;

    rawMatches.forEach((match, matchIndex) => {
      if (!match || typeof match !== "object") {
        diagnostics.ambiguousUnclassifiableRecords += 1;
        eventAmbiguousRecords += 1;
        return;
      }

      const recordKey = getWrestlingPersonDossierPrototypeRawMatchRecordKey(event, match, matchIndex);
      if (seenEmbeddedMatches.has(recordKey)) {
        diagnostics.duplicateMatchesRemoved += 1;
        return;
      }
      seenEmbeddedMatches.add(recordKey);
      diagnostics.dedupedEmbeddedMatchRecords += 1;
    });
    ...
    appearances.forEach((appearance, appearanceIndex) => {
      ...
      const hasSelectedPerson = hasWrestlingPersonDossierPrototypeSelectedPersonInWinParticipantMatch(match, context);
      if (isWrestlingPersonDossierPrototypeNonMatchSegment(match) && hasSelectedPerson) {
        diagnostics.nonMatchSegmentExclusions += 1;
        eventNonMatchPersonSegments += 1;
        return;
      }
      ...
      const recordKey = getWrestlingPersonDossierPrototypeMatchRecordKey(event, appearance, appearanceIndex);
      if (seenQualifyingMatches.has(recordKey)) {
        return;
      }
      seenQualifyingMatches.add(recordKey);

      const personSideKey = getWrestlingPersonDossierPrototypeSelectedPersonSideKey(match, context);
      const personSideValues = personSideKey ? getWrestlingPersonDossierPrototypeMatchSide(match, personSideKey) : [context.displayName];
      const explicitWinType = getWrestlingPersonDossierPrototypeExplicitWinType(match, personSideKey, personSideValues, context);
      diagnostics.qualifyingPersonMatches += 1;
      eventQualifyingMatches += 1;

      if (explicitWinType === "directPerson") {
        diagnostics.explicitDirectPersonWins += 1;
        return;
      }
      if (explicitWinType === "personSideTeam") {
        diagnostics.explicitPersonSideTeamWins += 1;
        return;
      }

      const participantType = getWrestlingPersonDossierPrototypeParticipantDiagnosticType(match, personSideKey);
      diagnostics.participant.total += 1;
      diagnostics.participant[participantType] += 1;
    });

    if (eventQualifyingMatches > 1) {
      diagnostics.eventBuckets.multiplePersonQualifyingMatchesFound += 1;
    } else if (eventQualifyingMatches === 1) {
      diagnostics.eventBuckets.personQualifyingMatchFound += 1;
    } else if (eventNonMatchPersonSegments > 0) {
      diagnostics.eventBuckets.nonMatchPersonSegmentOnly += 1;
    } else if (eventAmbiguousRecords > 0) {
      diagnostics.eventBuckets.ambiguousIncompleteMatchStructure += 1;
    } else {
      diagnostics.eventBuckets.embeddedMatchesNoQualifyingPersonMatch += 1;
    }
  });

  const loadedClassifiableTotal = diagnostics.explicitDirectPersonWins + diagnostics.explicitPersonSideTeamWins + diagnostics.participant.total;
  diagnostics.unreconciledDifference = Number.isFinite(diagnostics.peopleMatches)
    ? diagnostics.peopleMatches - loadedClassifiableTotal
    : null;
  diagnostics.visibleValue = diagnostics.visibleRecord.wins + " / " + diagnostics.visibleRecord.participant;
  diagnostics.calculatedValue = (diagnostics.explicitDirectPersonWins + diagnostics.explicitPersonSideTeamWins) + " / " + diagnostics.participant.total;
  diagnostics.calculationMatchesVisible = diagnostics.visibleValue === diagnostics.calculatedValue;
  return diagnostics;
}

function syncWrestlingPersonDossierPrototypeWinParticipantDiagnostics() {
  const state = wrestlingPersonDossierPrototypeEventHistoryState;
  if (state.status !== "loaded" || !Array.isArray(state.events) || state.events.length === 0) {
    state.diagnostics = null;
    return null;
  }

  const diagnostics = getWrestlingPersonDossierPrototypeWinParticipantDiagnostics(state.events, wrestlingPersonDossierPrototypeSelectedPersonState.record);
  state.diagnostics = diagnostics;
  const diagnosticsLogKey = JSON.stringify({
    totalTimelineEvents: diagnostics.totalTimelineEvents,
    rawEmbeddedMatchRecords: diagnostics.rawEmbeddedMatchRecords,
    qualifyingPersonMatches: diagnostics.qualifyingPersonMatches,
    calculatedValue: diagnostics.calculatedValue,
    peopleMatches: diagnostics.peopleMatches,
  });
  if (state.diagnosticsLogKey !== diagnosticsLogKey && typeof console !== "undefined" && typeof console.debug === "function") {
    state.diagnosticsLogKey = diagnosticsLogKey;
    console.debug("[wrestling-person-dossier-prototype] win-participant diagnostics", diagnostics);
  }
  return diagnostics;
}

function normalizeWrestlingPersonDossierPrototypePersonAppearance(match, matchIndex, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const personSideKey = getWrestlingPersonDossierPrototypeSelectedPersonSideKey(match, context);
  const opposingSideKey = personSideKey === "side_1" ? "side_2" : personSideKey === "side_2" ? "side_1" : "";
  const personSideValues = personSideKey ? getWrestlingPersonDossierPrototypeMatchSide(match, personSideKey) : [context.displayName];
  const opposingSideValues = (opposingSideKey ? getWrestlingPersonDossierPrototypeMatchSide(match, opposingSideKey) : [])
    .filter((value) => !isWrestlingPersonDossierPrototypeSelectedPersonIdentity(value, context));
  const matchOrder = Number(match?.match_order ?? match?.matchOrder ?? match?.order ?? matchIndex + 1);

  return {
    match,
    matchOrder: Number.isFinite(matchOrder) ? matchOrder : matchIndex + 1,
    personSide: formatWrestlingPersonDossierPrototypeEventHistoryList(personSideValues, context.displayName),
    ...
    result: getWrestlingPersonDossierPrototypeAppearanceResult(match, personSideKey, personSideValues, context),
  };
}

function normalizeWrestlingPersonDossierPrototypeEvent(row, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const matches = Array.isArray(row?.matches) ? row.matches : [];
  const personAppearances = matches
    .map((match, matchIndex) => ({ match, matchIndex }))
    .filter(({ match }) => match && typeof match === "object" && hasWrestlingPersonDossierPrototypeSelectedPersonInMatch(match, context))
    .map(({ match, matchIndex }) => normalizeWrestlingPersonDossierPrototypePersonAppearance(match, matchIndex, context))
    .sort((left, right) => left.matchOrder - right.matchOrder);

  if (matches.length > 0 && personAppearances.length === 0) {
    return null;
  }
...
function normalizeWrestlingPersonDossierPrototypeEventHistory(payload, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  const eventMap = new Map();
  getWrestlingPersonDossierPrototypeEventHistoryRows(payload).forEach((row) => {
    const event = normalizeWrestlingPersonDossierPrototypeEvent(row, context);
    if (!event) {
      return;
    }
    const existingEvent = eventMap.get(event.id);
    if (existingEvent) {
      existingEvent.personAppearances.push(...event.personAppearances);
      existingEvent.personAppearances.sort((left, right) => left.matchOrder - right.matchOrder);
      return;
    }
...
function setWrestlingPersonDossierPrototypeEventPreview(workspace, position, event) {
  const preview = workspace?.querySelector(`[data-wrestling-person-dossier-prototype-event-preview="${position}"]`);
  if (!preview) {
    return;
  }

  preview.classList.toggle("is-empty", !event);
  preview.setAttribute("aria-hidden", event ? "false" : "true");
  setWrestlingPersonDossierPrototypeText(preview, "[data-wrestling-person-dossier-prototype-event-preview-title]", event?.title || "");
  setWrestlingPersonDossierPrototypeText(preview, "[data-wrestling-person-dossier-prototype-event-preview-date]", event?.date || "");
...
function getWrestlingPersonDossierPrototypeSelectedEventArchiveContext() {
  const state = wrestlingPersonDossierPrototypeEventHistoryState;
  if (state.status !== "loaded" || !Array.isArray(state.events) || state.events.length === 0) {
    return null;
  }
...
function getWrestlingPersonDossierPrototypeEventArchivePhotoGrid(archive) {
  return archive?.querySelector("[data-wrestling-person-dossier-prototype-event-archive-photo-grid]");
}

function setWrestlingPersonDossierPrototypeEventArchivePhotoCount(archive, count) {
  const value = count === null || count === undefined || count === "" ? Number.NaN : Number(count);
  setWrestlingPersonDossierPrototypeText(
    archive,
    "[data-wrestling-person-dossier-prototype-event-archive-photos]",
    Number.isFinite(value) && value >= 0 ? new Intl.NumberFormat("en-US").format(value) : "\u2014"
  );
}

function getWrestlingPersonDossierPrototypeEventArchivePhotoPerson() {
...
function createWrestlingPersonDossierPrototypeEventArchivePhotoEventRow(event, appearance) {
  if (!event || !appearance?.match) {
    return null;
  }
  ...
  return {
    ...
    ...match,
    showId,
    show_id: raw.show_id || raw.showId || showId,
    eventId: getWrestlingText(raw.eventId || raw.event_id || showId),
    event_id: raw.event_id || raw.eventId || showId,
    dateKey,
    date_key: raw.date_key || raw.dateKey || dateKey,
    showKey: raw.showKey || raw.show_key || event.id || dateKey,
    show_key: raw.show_key || raw.showKey || event.id || dateKey,
    ...
    matchRef,
    match_ref: match.match_ref || match.matchRef || matchRef,
    matchId: match.matchId || match.match_id || match.id || matchRef,
    match_id: match.match_id || match.matchId || match.id || matchRef,
    matchName: match.matchName || match.match_name || match.title || appearance?.matchType,
    matchType: appearance?.matchType || match.matchType || match.match_type,
    personRole: appearance?.result || match.personRole || match.person_role,
    backend_record: match,
  };
}

function getWrestlingPersonDossierPrototypeEventArchivePhotoRequestKey(eventRow, person) {
  const personId = normalizeWrestlingPersonId(person?.personId || person?.name);
  const showId = normalizeWrestlingArchiveSlug(getWrestlingPersonEventRowPhotoShowId(eventRow), "");
  const matchRef = normalizeWrestlingArchiveSlug(getWrestlingPersonEventRowPhotoMatchRef(eventRow), "");
  return personId && showId && matchRef ? [personId, showId, matchRef].join("|") : "";
}

function getWrestlingPersonDossierPrototypeEventArchiveBody(archive) {
  return archive?.querySelector("[data-wrestling-person-dossier-prototype-event-archive-body]");
}

function getWrestlingPersonDossierPrototypeEventArchiveViewer(archive) {
  return archive?.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer]");
}

function getWrestlingPersonDossierPrototypeEventArchivePhotoCaption(photo) {
  return getWrestlingText(photo?.caption || photo?.Caption);
}

function getWrestlingPersonDossierPrototypeEventArchiveHeroPhotoUrl(photo) {
...
function parseWrestlingPersonDossierPrototypeEventArchivePhotoDate(value) {
  ...
  const exifMatch = rawValue.match(/^(\d{4}):(\d{2}):(\d{2})\s+(\d{2}:\d{2}:\d{2})/);
  if (!exifMatch) {
    return 0;
  }

  const exifTimestamp = Date.parse(`${exifMatch[1]}-${exifMatch[2]}-${exifMatch[3]}T${exifMatch[4]}`);
  return Number.isFinite(exifTimestamp) ? exifTimestamp : 0;
}

function getWrestlingPersonDossierPrototypeEventArchivePhotoDateSort(photo) {
  const fields = ["date_time_original", "dateTimeOriginal", "DateTimeOriginal", "taken_at", "takenAt", "date_taken", "dateTaken", "created_at", "createdAt", "date"];
  for (const field of fields) {
    const timestamp = parseWrestlingPersonDossierPrototypeEventArchivePhotoDate(photo?.[field]);
    if (timestamp > 0) {
      return { timestamp, field };
    }
  }
  return { timestamp: 0, field: "" };
}

function getWrestlingPersonDossierPrototypeEventArchivePhotoNumericSort(photo) {
  const fields = ["sort_order", "sortOrder", "display_order", "displayOrder", "order", "position", "photo_index", "photoIndex", "image_index", "imageIndex", "index", "sequence", "sequence_number", "sequenceNumber"];
  for (const field of fields) {
    const value = Number.parseFloat(photo?.[field]);
    if (Number.isFinite(value) && value >= 0) {
      return { value, field };
    }
  }
  return { value: Number.NaN, field: "" };
}

function getWrestlingPersonDossierPrototypeEventArchiveFilteredPhotos(match, person) {
  const source = getWrestlingPersonEventRowLiveMatch(match) || match;
  const matchNames = getWrestlingPersonCaptionMatchNames(person);
  return getWrestlingMatchPhotoItems(source)
    .map((photo, index) => ({
      ...photo,
      __wrestlingPersonDossierPrototypeSourceIndex: index,
    }))
    .filter((photo) => getWrestlingPersonMatchedPhotoUrl(photo))
    .filter((photo) => doesWrestlingPersonCaptionMatch(photo, matchNames));
}

function sortWrestlingPersonDossierPrototypeEventArchivePhotos(photos) {
  return (Array.isArray(photos) ? photos : [])
    .map((photo, index) => {
      const dateSort = getWrestlingPersonDossierPrototypeEventArchivePhotoDateSort(photo);
      const numericSort = getWrestlingPersonDossierPrototypeEventArchivePhotoNumericSort(photo);
      const sourceIndex = Number(photo?.__wrestlingPersonDossierPrototypeSourceIndex);
      return {
        photo,
        index,
        timestamp: dateSort.timestamp,
        orderValue: numericSort.value,
        sourceIndex: Number.isFinite(sourceIndex) ? sourceIndex : index,
      };
    })
    .sort((left, right) => {
      if (left.timestamp > 0 || right.timestamp > 0) {
        const leftTimestamp = left.timestamp > 0 ? left.timestamp : Number.MAX_SAFE_INTEGER;
        const rightTimestamp = right.timestamp > 0 ? right.timestamp : Number.MAX_SAFE_INTEGER;
        if (leftTimestamp !== rightTimestamp) {
          return leftTimestamp - rightTimestamp;
        }
      }

      const leftHasOrder = Number.isFinite(left.orderValue);
      const rightHasOrder = Number.isFinite(right.orderValue);
      if (leftHasOrder || rightHasOrder) {
        const leftOrder = leftHasOrder ? left.orderValue : Number.MAX_SAFE_INTEGER;
        const rightOrder = rightHasOrder ? right.orderValue : Number.MAX_SAFE_INTEGER;
...
function getWrestlingPersonDossierPrototypeEventArchivePositionText(index, total) {
  const width = Math.max(2, String(total || 0).length);
  return `${String(index + 1).padStart(width, "0")} of ${String(total).padStart(width, "0")}`;
}

function setWrestlingPersonDossierPrototypeEventArchiveMode(archive, mode) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const normalizedMode = mode === "viewer" ? "viewer" : "grid";
  const body = getWrestlingPersonDossierPrototypeEventArchiveBody(archive);
  const viewer = getWrestlingPersonDossierPrototypeEventArchiveViewer(archive);
  state.mode = normalizedMode;
  if (body) {
    body.dataset.wrestlingPersonDossierPrototypeEventArchiveMode = normalizedMode;
  }
  if (viewer) {
    viewer.hidden = normalizedMode !== "viewer";
  }
}

let wrestlingPersonEventArchivePhotoActions = null;

function syncWrestlingPersonEventArchivePhotoActions(viewer, photo, selectedIndex, viewerImageRequestKey, downloadPhoto = photo) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const figure = viewer.querySelector(".wrestling-person-dossier-prototype-event-archive__viewer-figure");
  if (!figure) return;
  if (!wrestlingPersonEventArchivePhotoActions || wrestlingPersonEventArchivePhotoActions.element.parentElement !== figure) {
    wrestlingPersonEventArchivePhotoActions?.cancel();
    wrestlingPersonEventArchivePhotoActions = createLightboxPhotoActions({
      isCurrent: () => state.isOpen && state.mode === "viewer" && !viewer.hidden,
      noteId: "v3-person-photo-download-note",
    });
    figure.append(wrestlingPersonEventArchivePhotoActions.element);
  }
  wrestlingPersonEventArchivePhotoActions.sync({
    ...getWrestlingPhotoDownloadData(downloadPhoto, selectedIndex, getWrestlingText(photo.matchRef || photo.match_ref || photo.match_id || photo.matchId, "person")),
    galleryLightboxId: `${viewerImageRequestKey}:${selectedIndex}:${getWrestlingText(photo.image_key || photo.ImageKey || photo.photoId)}`,
    wrestlingLightboxRoute: location.href,
  });
}
...
function renderWrestlingPersonDossierPrototypeEventArchiveViewer(archive) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const viewer = getWrestlingPersonDossierPrototypeEventArchiveViewer(archive);
  const photos = Array.isArray(state.photoRenderItems) ? state.photoRenderItems : [];
  const photo = photos[state.selectedPhotoIndex];
  if (!viewer || !photo) {
    return false;
  }

  const image = viewer.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer-image]");
  const caption = viewer.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer-caption]");
  ...
  syncWrestlingPersonEventArchivePhotoActions(viewer, photo, selectedIndex, viewerImageRequestKey);
  if (image) {
    ...
    if (!getWrestlingText(photo.large_url || photo.largeUrl || photo.medium_url || photo.mediumUrl)) {
      requestWrestlingPersonSelectedPhotoDetail(photo).then(async (detail) => {
        if (detail && state.viewerImageRequestKey === viewerImageRequestKey && state.selectedPhotoIndex === selectedIndex && state.mode === "viewer" && state.isOpen && state.photoRenderItems[selectedIndex] === photo) {
          syncWrestlingPersonEventArchivePhotoActions(viewer, photo, selectedIndex, viewerImageRequestKey, { ...photo, ...detail });
        }
...
function openWrestlingPersonDossierPrototypeEventArchiveViewer(archive, photoIndex) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const photos = Array.isArray(state.photoRenderItems) ? state.photoRenderItems : [];
  const selectedIndex = Number.parseInt(photoIndex, 10);
  if (!archive || !Number.isFinite(selectedIndex) || !photos[selectedIndex]) {
    return false;
  }

  const region = getWrestlingPersonDossierPrototypeEventArchivePhotoGrid(archive);
  state.photoGridScrollTop = region?.scrollTop || 0;
  state.viewerReturnPhotoIndex = selectedIndex;
  state.selectedPhotoIndex = selectedIndex;
  setWrestlingPersonDossierPrototypeEventArchiveMode(archive, "viewer");
  if (!renderWrestlingPersonDossierPrototypeEventArchiveViewer(archive)) {
    setWrestlingPersonDossierPrototypeEventArchiveMode(archive, "grid");
    return false;
  }

  const focusTarget = archive.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer-close]") || getWrestlingPersonDossierPrototypeEventArchiveViewer(archive);
  window.requestAnimationFrame(() => {
...
function closeWrestlingPersonDossierPrototypeEventArchiveViewer(archive = getWrestlingPersonDossierPrototypeEventArchive()) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  if (!archive || state.mode !== "viewer") {
    return false;
  }

  const photos = Array.isArray(state.photoRenderItems) ? state.photoRenderItems : [];
  const originIndex = state.viewerReturnPhotoIndex;
  const returnIndex = state.selectedPhotoIndex;
  const scrollTop = state.photoGridScrollTop;
  const originPage = getWrestlingPersonDossierPrototypeEventArchivePhotoPageForIndex(originIndex);
  const returnPage = getWrestlingPersonDossierPrototypeEventArchivePhotoPageForIndex(returnIndex);
  setWrestlingPersonDossierPrototypeEventArchiveMode(archive, "grid");
  state.photoGalleryPage = getWrestlingPersonDossierPrototypeEventArchiveBoundedPhotoPage(returnPage, photos);
  renderWrestlingPersonDossierPrototypeEventArchivePhotoPage(archive, { animate: false, scrollToTop: false });
  resetWrestlingPersonDossierPrototypeEventArchiveViewerState();
  const region = getWrestlingPersonDossierPrototypeEventArchivePhotoGrid(archive);
  window.requestAnimationFrame(() => {
    if (region) {
      region.scrollTop = returnPage === originPage ? scrollTop : 0;
    }
    const focusTarget = archive.querySelector(`[data-wrestling-person-dossier-prototype-event-archive-photo-index="${returnIndex}"]`) || region;
    focusTarget?.focus?.({ preventScroll: true });
  });
  return true;
}

function moveWrestlingPersonDossierPrototypeEventArchiveViewerPhoto(delta, archive = getWrestlingPersonDossierPrototypeEventArchive()) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const photos = Array.isArray(state.photoRenderItems) ? state.photoRenderItems : [];
  if (!archive || state.mode !== "viewer" || photos.length === 0) {
    return false;
  }

  const nextIndex = Math.min(Math.max(state.selectedPhotoIndex + delta, 0), photos.length - 1);
  if (nextIndex === state.selectedPhotoIndex) {
    return false;
  }

  state.selectedPhotoIndex = nextIndex;
...
function getWrestlingPersonDossierPrototypeEventArchiveBoundedPhotoPage(page, photos) {
  ...
  return Math.min(Math.max(Number.isFinite(value) ? value : 1, 1), pageCount);
}

function resetWrestlingPersonDossierPrototypeEventArchivePhotoGallery(region) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  state.photoRenderItems = [];
  state.photoGalleryPage = 1;
  if (region) {
    region.onscroll = null;
  }
}

function createWrestlingPersonDossierPrototypeEventArchivePhotoTile(photo, index, pageIndex = index) {
  const imageSrc = getWrestlingPersonMatchedPhotoUrl(photo);
  const tile = document.createElement("button");
  tile.className = "wrestling-person-dossier-prototype-event-archive__photo-tile";
  tile.type = "button";
  tile.dataset.wrestlingPersonDossierPrototypeEventArchivePhotoIndex = String(index);
  tile.setAttribute("aria-label", getWrestlingText(photo?.label || photo?.caption || photo?.Caption, `${getWrestlingPersonDossierPrototypeSelectedPersonContext().displayName} event archive photo ${index + 1}`));
  tile.addEventListener("click", () => {
    openWrestlingPersonDossierPrototypeEventArchiveViewer(getWrestlingPersonDossierPrototypeEventArchive(), index);
  });

  const image = document.createElement("img");
  image.className = "wrestling-person-dossier-prototype-event-archive__photo-image";
  image.src = imageSrc;
  image.alt = "";
  image.loading = pageIndex < 8 ? "eager" : "lazy";
  image.decoding = "async";
  tile.append(image);
...
function createWrestlingPersonDossierPrototypeEventArchivePhotoPageButton(label, isEnabled) {
  const button = document.createElement("button");
  button.className = "wrestling-person-dossier-prototype-event-archive__photo-page-button";
  button.type = "button";
  button.textContent = label;
  setWrestlingPersonDossierPrototypeControlEnabled(button, isEnabled, "");
  return button;
}

function renderWrestlingPersonDossierPrototypeEventArchivePhotoPage(archive, options = {}) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const region = getWrestlingPersonDossierPrototypeEventArchivePhotoGrid(archive);
  const photos = Array.isArray(state.photoRenderItems) ? state.photoRenderItems : [];
  if (!archive || !region || photos.length === 0) {
    return false;
  }
  ...
  pagePhotos.forEach((photo, pageIndex) => {
    grid.append(createWrestlingPersonDossierPrototypeEventArchivePhotoTile(photo, startIndex + pageIndex, pageIndex));
  });
  region.append(grid);

  if (pageCount > 1) {
    const pagination = document.createElement("nav");
    pagination.className = "wrestling-person-dossier-prototype-event-archive__photo-pagination";
    pagination.setAttribute("aria-label", "Event archive photo pages");

    const previous = createWrestlingPersonDossierPrototypeEventArchivePhotoPageButton("Previous", page > 1);
    previous.dataset.wrestlingPersonDossierPrototypeEventArchivePhotoPagePrevious = "true";
    previous.addEventListener("click", () => {
      moveWrestlingPersonDossierPrototypeEventArchivePhotoPage(-1, archive);
    });

    const indicator = document.createElement("span");
    indicator.className = "wrestling-person-dossier-prototype-event-archive__photo-page-indicator";
    indicator.textContent = `${page} of ${pageCount}`;
    indicator.setAttribute("aria-live", "polite");

    const next = createWrestlingPersonDossierPrototypeEventArchivePhotoPageButton("Next", page < pageCount);
    next.dataset.wrestlingPersonDossierPrototypeEventArchivePhotoPageNext = "true";
  ...
  setWrestlingPersonDossierPrototypeEventArchivePhotoCount(archive, photos.length);
...
function moveWrestlingPersonDossierPrototypeEventArchivePhotoPage(delta, archive = getWrestlingPersonDossierPrototypeEventArchive()) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const photos = Array.isArray(state.photoRenderItems) ? state.photoRenderItems : [];
  ...
  const currentPage = getWrestlingPersonDossierPrototypeEventArchiveBoundedPhotoPage(state.photoGalleryPage, photos);
  const nextPage = getWrestlingPersonDossierPrototypeEventArchiveBoundedPhotoPage(currentPage + delta, photos);
  if (nextPage === currentPage) {
    return false;
  }

  return renderWrestlingPersonDossierPrototypeEventArchivePhotoPage(archive, { page: nextPage });
}

function setWrestlingPersonDossierPrototypeEventArchivePhotoStatus(archive, stateName, photos = []) {
  const region = getWrestlingPersonDossierPrototypeEventArchivePhotoGrid(archive);
  if (!region) {
    return;
  }
  ...
  if (stateName === "loaded" && photos.length > 0) {
    wrestlingPersonDossierPrototypeEventArchiveState.photoRenderItems = photos;
    wrestlingPersonDossierPrototypeEventArchiveState.photoGalleryPage = 1;
    renderWrestlingPersonDossierPrototypeEventArchivePhotoPage(archive, { animate: false });
    return;
  }
...
function loadWrestlingPersonDossierPrototypeEventArchivePhotos(archive, context) {
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const eventRow = createWrestlingPersonDossierPrototypeEventArchivePhotoEventRow(context?.event, context?.appearance);
  const person = getWrestlingPersonDossierPrototypeEventArchivePhotoPerson();
  const requestKey = getWrestlingPersonDossierPrototypeEventArchivePhotoRequestKey(eventRow, person);
  if (!eventRow || !requestKey) {
    state.photoRequestKey = "";
    setWrestlingPersonDossierPrototypeEventArchivePhotoStatus(archive, "empty", []);
    return Promise.resolve([]);
  }

  state.photoRequestKey = requestKey;
  const cachedPhotos = state.photoCache.get(requestKey);
  if (cachedPhotos) {
  ...
  return request
    .then((photos) => {
      if (!state.isOpen || state.photoRequestKey !== requestKey || state.eventIndex !== context?.eventIndex) {
        return photos;
      }
      setWrestlingPersonDossierPrototypeEventArchivePhotoStatus(archive, photos.length > 0 ? "loaded" : "empty", photos);
      return photos;
    })
    .catch(() => {
      if (state.isOpen && state.photoRequestKey === requestKey && state.eventIndex === context?.eventIndex) {
        setWrestlingPersonDossierPrototypeEventArchivePhotoStatus(archive, "error", []);
      }
      return [];
    });
}

function renderWrestlingPersonDossierPrototypeEventArchiveContext(archive, context) {
  const event = context?.event || null;
  const appearance = context?.appearance || null;
  if (!archive || !event) {
    return false;
  }
  ...
  setWrestlingPersonDossierPrototypeEventArchiveMode(archive, "grid");
  setWrestlingPersonDossierPrototypeEventArchivePhotoStatus(archive, "loading", []);
  setWrestlingPersonDossierPrototypeText(archive, "[data-wrestling-person-dossier-prototype-event-archive-event-title]", event.title || "Event Title Unlisted");
  setWrestlingPersonDossierPrototypeText(archive, "[data-wrestling-person-dossier-prototype-event-archive-event-date]", event.date || "Date Unlisted");
  setWrestlingPersonDossierPrototypeText(archive, "[data-wrestling-person-dossier-prototype-event-archive-event-venue]", event.venue || "Venue Unlisted");
  setWrestlingPersonDossierPrototypeText(archive, "[data-wrestling-person-dossier-prototype-event-archive-event-location]", event.location || "Location Unlisted");
  setWrestlingPersonDossierPrototypeText(archive, "[data-wrestling-person-dossier-prototype-event-archive-match-type]", appearance?.matchType || "Match Type Unlisted");
  setWrestlingPersonDossierPrototypeText(archive, "[data-wrestling-person-dossier-prototype-event-archive-opponent]", appearance?.opposingSide || "Opponent Unlisted");
  setWrestlingPersonDossierPrototypeText(archive, "[data-wrestling-person-dossier-prototype-event-archive-result]", appearance?.result || "Result Unlisted");
  return true;
}
...
function handleWrestlingPersonDossierPrototypeEventArchiveKeydown(event) {
  ...
  if (event.key === "Escape") {
    ...
    if (state.mode === "viewer") {
      closeWrestlingPersonDossierPrototypeEventArchiveViewer();
      return;
    }
  ...
  if (state.mode === "viewer" && (event.key === "ArrowLeft" || event.key === "ArrowRight")) {
    event.preventDefault();
    moveWrestlingPersonDossierPrototypeEventArchiveViewerPhoto(event.key === "ArrowLeft" ? -1 : 1);
    return;
  }
...
function openWrestlingPersonDossierPrototypeEventArchive(shell = wrestlingPersonDossierPrototypeShell, trigger = null) {
  ...
  const state = wrestlingPersonDossierPrototypeEventArchiveState;
  const context = getWrestlingPersonDossierPrototypeSelectedEventArchiveContext();
  if (!archive || !dialog || !context || !renderWrestlingPersonDossierPrototypeEventArchiveContext(archive, context)) {
    return false;
  }
...
function bindWrestlingPersonDossierPrototypeEventHistoryInteractions(shell = wrestlingPersonDossierPrototypeShell) {
  ...
  const viewer = shell.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer]");
  viewer?.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer-close]")?.addEventListener("click", (event) => {
    event.preventDefault();
    closeWrestlingPersonDossierPrototypeEventArchiveViewer();
  });
  viewer?.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer-previous]")?.addEventListener("click", (event) => {
    event.preventDefault();
    moveWrestlingPersonDossierPrototypeEventArchiveViewerPhoto(-1);
  });
  viewer?.querySelector("[data-wrestling-person-dossier-prototype-event-archive-viewer-next]")?.addEventListener("click", (event) => {
    event.preventDefault();
    moveWrestlingPersonDossierPrototypeEventArchiveViewerPhoto(1);
  });
  viewer?.addEventListener("pointerdown", (event) => {
    if (wrestlingPersonDossierPrototypeEventArchiveState.mode !== "viewer" || !event.isPrimary || (typeof event.button === "number" && event.button !== 0)) {
      return;
    }
    wrestlingPersonDossierPrototypeEventArchiveState.viewerSwipeStart = {
      pointerId: event.pointerId,
      x: event.clientX,
      y: event.clientY,
    };
  ...
  viewer?.addEventListener("pointerup", (event) => {
    const state = wrestlingPersonDossierPrototypeEventArchiveState;
    const swipeStart = state.viewerSwipeStart;
    if (state.mode !== "viewer" || !swipeStart || event.pointerId !== swipeStart.pointerId) {
      return;
    }
...
function renderWrestlingPersonDossierPrototypeEventHistoryShell(workspace, stateName, event = null, total = 0, events = [], activeIndex = 0, context = getWrestlingPersonDossierPrototypeSelectedPersonContext()) {
  ...
  setWrestlingPersonDossierPrototypeText(workspace, "[data-wrestling-person-dossier-prototype-event-opponents]", `vs ${selectedAppearance?.opposingSide || "Opponent Unlisted"}`);
...
function renderWrestlingPersonDossierPrototypeSelectedPersonState(shell = wrestlingPersonDossierPrototypeShell) {
  ...
  if (state.status === "loaded" && state.record) {
    ...
    setWrestlingPersonDossierPrototypeText(workspace, "[data-wrestling-person-dossier-prototype-name]", context.displayName);
...
function loadWrestlingPersonDossierPrototypeSelectedPersonRecord(shell = wrestlingPersonDossierPrototypeShell) {
  ...
  state.requestActivation = activation;
  renderWrestlingPersonDossierPrototypeSelectedPersonState(shell);
  const rawRequest = getWrestlingPersonDossierPrototypeRecordRequest(context, cacheKey);
  const ownsRequest = () => isCurrentWrestlingPersonDossierRequest(requestKey, activation) &&
    state.requestKey === requestKey && state.requestPromise === consumer;
  const consumer = rawRequest.then((result) => {
...
function createWrestlingPersonDossierPrototypeHallPresentation() {
  ...
  expandedHologram.innerHTML = `
    <section class="hall-of-champions-workspace hall-of-champions-workspace--dossier-prototype" data-wrestling-person-dossier-prototype-workspace="true" data-wrestling-person-dossier-prototype-state="loading" aria-label="Selected person modular dossier prototype">
      <div class="wrestling-person-dossier-prototype-framework" aria-labelledby="wrestling-person-dossier-prototype-name">
        ...
        <section class="wrestling-person-dossier-prototype-module wrestling-person-dossier-prototype-module--timeline" aria-label="Timeline event history module" data-wrestling-person-dossier-prototype-event-history-state="loading">
          ...
          <div class="wrestling-person-dossier-prototype-event-history" aria-label="Event history carousel shell">
            ...
            <article class="wrestling-person-dossier-prototype-event-history__card" aria-label="Selected person event history preview" data-wrestling-person-dossier-prototype-event-card>
      ...
      <div class="wrestling-person-dossier-prototype-event-archive" data-wrestling-person-dossier-prototype-event-archive data-wrestling-person-dossier-prototype-event-archive-state="closed" aria-hidden="true" hidden>
        ...
        <section class="wrestling-person-dossier-prototype-event-archive__dialog" role="dialog" aria-modal="true" aria-labelledby="wrestling-person-dossier-prototype-event-archive-title" tabindex="-1" data-wrestling-person-dossier-prototype-event-archive-dialog>
          ...
          <div class="wrestling-person-dossier-prototype-event-archive__body" data-wrestling-person-dossier-prototype-event-archive-body data-wrestling-person-dossier-prototype-event-archive-mode="grid">
            <section class="wrestling-person-dossier-prototype-event-archive__region wrestling-person-dossier-prototype-event-archive__region--summary" aria-label="Event archive details">
              ...
              <dl class="wrestling-person-dossier-prototype-event-archive__facts">
                <div>
                  <dt>MATCH TYPE</dt>
                  <dd data-wrestling-person-dossier-prototype-event-archive-match-type></dd>
                </div>
                <div>
                  <dt>OPPONENT</dt>
                  <dd data-wrestling-person-dossier-prototype-event-archive-opponent></dd>
                </div>
                <div>
                  <dt>RESULT</dt>
                  <dd data-wrestling-person-dossier-prototype-event-archive-result></dd>
                </div>
                <div>
                  <dt>PHOTOS</dt>
                  <dd data-wrestling-person-dossier-prototype-event-archive-photos></dd>
                </div>
              </dl>
            </section>
            <section class="wrestling-person-dossier-prototype-event-archive__region wrestling-person-dossier-prototype-event-archive__region--photos" aria-label="Event archive photo grid" data-wrestling-person-dossier-prototype-event-archive-photo-grid tabindex="-1"></section>
            <section class="wrestling-person-dossier-prototype-event-archive__viewer" aria-label="Event archive photo viewer" data-wrestling-person-dossier-prototype-event-archive-viewer tabindex="-1" hidden>
              <button class="wrestling-person-dossier-prototype-event-archive__viewer-close" type="button" aria-label="Return to Event Archive gallery" data-wrestling-person-dossier-prototype-event-archive-viewer-close>&times;</button>
              <button class="wrestling-person-dossier-prototype-event-archive__viewer-nav wrestling-person-dossier-prototype-event-archive__viewer-nav--previous" type="button" aria-label="Previous photo" data-wrestling-person-dossier-prototype-event-archive-viewer-previous>&lsaquo;</button>
              <figure class="wrestling-person-dossier-prototype-event-archive__viewer-figure">
                <div class="wrestling-person-dossier-prototype-event-archive__viewer-stage">
                  <img class="wrestling-person-dossier-prototype-event-archive__viewer-image" data-wrestling-person-dossier-prototype-event-archive-viewer-image alt="" decoding="async">
                </div>
                <figcaption class="wrestling-person-dossier-prototype-event-archive__viewer-caption" data-wrestling-person-dossier-prototype-event-archive-viewer-caption hidden></figcaption>
              </figure>
              <button class="wrestling-person-dossier-prototype-event-archive__viewer-nav wrestling-person-dossier-prototype-event-archive__viewer-nav--next" type="button" aria-label="Next photo" data-wrestling-person-dossier-prototype-event-archive-viewer-next>&rsaquo;</button>
              <p class="wrestling-person-dossier-prototype-event-archive__viewer-position" data-wrestling-person-dossier-prototype-event-archive-viewer-position></p>
            </section>
          </div>
        </section>
      </div>
...
function getWrestlingPersonDossierPrototypeShell() {
  ...
  wrestlingPersonDossierPrototypeShell = document.querySelector("[data-wrestling-person-dossier-prototype-shell]");
  if (wrestlingPersonDossierPrototypeShell) {
    return wrestlingPersonDossierPrototypeShell;
  }
  ...
  const shellElement = document.querySelector(".site-shell");
  (shellElement || document.body).appendChild(prototypeShell);
  wrestlingPersonDossierPrototypeShell = prototypeShell;
  return wrestlingPersonDossierPrototypeShell;
}
...
function setWrestlingPersonDossierPrototypeActive(isActive, subject = null) {
  ...
  if (isActive) {
    ...
    const snapshot = hallOfChampionsReturnSnapshots.get(token);
    hallOfChampionsPersonReturn = window.history.state?.fromWrestlingPeopleHall && snapshot?.personPath === location.pathname
      ? { token, sourcePath: snapshot.sourcePath } : null;
  } else if (hallOfChampionsPersonReturn?.sourcePath !== location.pathname) {
    hallOfChampionsPersonReturn = null;
  }
  ...
  if (isActive) {
    const context = setWrestlingPersonDossierPrototypeSelectedSubject(subject || {});
    const requestKey = context.stableId || context.normalizedDisplayName;
    const state = wrestlingPersonDossierPrototypeSelectedPersonState;
    // Begin the same authoritative request before constructing or measuring the dossier.
    if (isCurrentWrestlingPersonDossierRequest("", activation) &&
      !(["loaded", "empty"].includes(state.status) && state.requestKey === requestKey) &&
      !getWrestlingPersonDossierPrototypeCachedRecord(context, requestKey)) {
      getWrestlingPersonDossierPrototypeRecordRequest(context, requestKey);
    }
  }
  const shellElement = document.querySelector(".site-shell");
  const prototypeShell = isActive
  ...
  if (shellElement) {
    if (isActive) {
      shellElement.dataset.wrestlingPersonDossierPrototype = "blank";
      shellElement.dataset.portfolioGatewayWorld = "battleground";
      shellElement.dataset.portfolioGatewayRoute = "/wrestling";
      shellElement.dataset.portfolioGatewayState = "filling-screen";
      shellElement.dataset.activeWorld = "battleground";
...
function getWrestlingPersonDossierRouteSubject(personSlug) {
  const slug = normalizeWrestlingPersonId(personSlug);
  const displayName = getWrestlingPersonDossierPrototypeDisplayValue(String(personSlug || "").replace(/[-_]+/g, " "), slug || "Person");
  return {
    slug,
    displayName,
...
function showWrestlingPersonDossierRoute(personSlug) {
  setWrestlingPersonDossierPrototypeActive(true, getWrestlingPersonDossierRouteSubject(personSlug));
}
...
function getWrestlingPhotoDownloadData(photo, index = 0, matchRef = "match-1") {
  const galleryDownloadUrl = getWrestlingMatchPhotoDownloadUrl(photo);
  const downloadExtension = galleryDownloadUrl.match(/\.(jpe?g|png|webp|avif|gif)(?:[?#]|$)/i)?.[1] || "";
  ...
  const galleryDownloadFilename = `vmpix-${downloadMatch}-${downloadName}${downloadExtension ? `.${downloadExtension}` : ""}`;
  return { galleryDownloadUrl, galleryDownloadFilename };
}
...
function createWrestlingMatchPhotoLightboxTile(photo, index = 0, show = {}, match = {}, options = {}) {
  ...
  Object.assign(tile.dataset, getWrestlingPhotoDownloadData(photo, index, matchRef));
  tile.dataset.galleryKind = "image";
  tile.dataset.galleryMediaId = imageKey;
