...
function getWrestlingPeoplePrototypeShell() {
  ...
  expandedHologram.innerHTML = `
    ...
    <section class="hall-of-champions-az-workspace" data-hall-of-champions-az-workspace aria-label="Hall of Champions alphabetical index workspace" aria-hidden="true" hidden>
      ...
      <div class="hall-of-champions-az-workspace__results" data-hall-of-champions-az-results role="region" aria-label="Alphabetical archive records" aria-live="polite" aria-busy="false">
        <p class="hall-of-champions-az-workspace__result-count" data-hall-of-champions-az-result-count aria-live="polite">INDEXING RECORDS</p>
        <div class="hall-of-champions-az-workspace__result-scroll" data-hall-of-champions-az-result-scroll>
          <p class="hall-of-champions-az-workspace__message" role="status">INDEXING RECORDS</p>
        </div>
      </div>
    ...
    <section class="hall-of-champions-category-workspace" data-hall-of-champions-category-workspace aria-label="Hall of Champions archive categories workspace" aria-hidden="true" hidden>
      <div class="hall-of-champions-category-workspace__header">
    ...
    <section class="hall-of-champions-team-workspace" data-hall-of-champions-team-workspace aria-label="Hall of Champions team directory workspace" aria-hidden="true" hidden>
      ...
      <p class="hall-of-champions-team-workspace__status" data-hall-of-champions-team-readout aria-live="polite" aria-atomic="true">CURRENT TEAM // DISCOVERING</p>
      <div class="hall-of-champions-az-workspace__results hall-of-champions-team-workspace__results" data-hall-of-champions-team-results role="region" aria-label="Team archive records" aria-live="polite" aria-busy="false">
        <p class="hall-of-champions-az-workspace__result-count hall-of-champions-team-workspace__result-count" data-hall-of-champions-team-result-count aria-live="polite">0 RECORDS</p>
        <div class="hall-of-champions-az-workspace__result-scroll hall-of-champions-team-workspace__result-scroll" data-hall-of-champions-team-result-scroll>
          <p class="hall-of-champions-az-workspace__message hall-of-champions-team-workspace__message" role="status">NO ARCHIVE RECORDS</p>
        </div>
      </div>
    </section>
  `;
...
function syncHallOfChampionsProjectionGeometry(prototypeShell) {
  ...
  const crystal = prototypeShell.querySelector(".hall-of-champions-pedestal__crystal");
  const pedestal = prototypeShell.querySelector("[data-hall-of-champions-pedestal]");
...
function getHallOfChampionsPeopleArchiveApiUrl(page = 1) {
  const apiUrl = new URL(WRESTLING_PEOPLE_API_ROUTE, WRESTLING_SHOWS_API_BASE_URL);
  apiUrl.searchParams.set("limit", String(HALL_OF_CHAMPIONS_PEOPLE_ARCHIVE_LIMIT));
...
function getHallOfChampionsPeopleArchivePagination(payload) {
  return payload?.pagination || payload?.meta?.pagination || payload?.meta || {};
}

function getHallOfChampionsPeopleArchiveTotalPages(payload) {
  const pagination = getHallOfChampionsPeopleArchivePagination(payload);
  const explicitTotalPages = Number.parseInt(
    pagination.totalPages || pagination.total_pages || payload?.totalPages || payload?.total_pages,
    10
  );
  if (Number.isFinite(explicitTotalPages) && explicitTotalPages > 0) {
    return explicitTotalPages;
  ...
  const pageLimit = Number.parseInt(pagination.limit || payload?.limit, 10) || WRESTLING_PEOPLE_API_LIMIT;
  return Number.isFinite(totalRows) && totalRows > pageLimit
    ? Math.ceil(totalRows / pageLimit)
    : 1;
}

function getHallOfChampionsPeopleArchiveTotalRecords(payload) {
  const pagination = getHallOfChampionsPeopleArchivePagination(payload);
  const totalRows = Number.parseInt(pagination.total || payload?.total || payload?.count, 10);
...
function getHallOfChampionsPeopleArchiveDisplayName(record) {
  return getWrestlingText(record?.name).trim();
}
...
function getHallOfChampionsPeopleArchiveKey(record, displayName) {
...
function discoverHallOfChampionsPeopleArchiveTeams() {
  const selectedTeamKey = hallOfChampionsPeopleArchiveTeams[hallOfChampionsTeamIndex]?.toLowerCase() || "";
  ...
  if (selectedTeamKey) {
    const preservedIndex = hallOfChampionsPeopleArchiveTeams.findIndex((teamName) => teamName.toLowerCase() === selectedTeamKey);
    hallOfChampionsTeamIndex = preservedIndex >= 0 ? preservedIndex : 0;
  } else {
...
function hydrateHallOfChampionsPeopleArchiveCache(prototypeShell) {
  ...
  syncHallOfChampionsTeamWorkspace(prototypeShell);
  syncHallOfChampionsTeamSelectorWhenActive(prototypeShell);
...
function mergeHallOfChampionsPeopleArchivePayload(prototypeShell, payload) {
  ...
  renderHallOfChampionsTeamResults(prototypeShell);
}
...
function getHallOfChampionsTeamMatches() {
  ...
  return hallOfChampionsPeopleArchiveRecords.filter((record) => {
    try {
      return getHallOfChampionsPeopleArchiveDiscoveredTeams(record)
        .some((teamName) => getHallOfChampionsTeamFilterKey(teamName) === selectedTeamKey);
    } catch (error) {
      reportHallOfChampionsPeopleArchiveRecordIssue("team filter skipped record", error, record);
      return false;
    }
  });
}
...
function renderHallOfChampionsAzResults(prototypeShell) {
  ...
  const scrollElement = prototypeShell?.querySelector("[data-hall-of-champions-az-result-scroll]");
  if (!resultsRegion || !countElement || !scrollElement) {
    return;
  }

  const selectedLetter = getHallOfChampionsCurrentAzLetter();
  const matches = getHallOfChampionsAzMatches();
  const fragment = document.createDocumentFragment();
  resultsRegion.dataset.hallOfChampionsAzResultsState = hallOfChampionsPeopleArchiveState;
  ...
  if (hallOfChampionsPeopleArchiveState === "loading" && matches.length > 0) {
    ...
    retryButton.className = "hall-of-champions-az-workspace__retry";
    retryButton.type = "button";
    retryButton.textContent = "RETRY";
    retryButton.addEventListener("click", handleHallOfChampionsPeopleArchiveRetry);
...
function renderHallOfChampionsTeamResults(prototypeShell) {
  const resultsRegion = prototypeShell?.querySelector("[data-hall-of-champions-team-results]");
  const countElement = prototypeShell?.querySelector("[data-hall-of-champions-team-result-count]");
  const scrollElement = prototypeShell?.querySelector("[data-hall-of-champions-team-result-scroll]");
  if (!resultsRegion || !countElement || !scrollElement) {
    return;
  }

  const selectedTeam = getHallOfChampionsCurrentTeam();
  const matches = getHallOfChampionsTeamMatches();
  const fragment = document.createDocumentFragment();
  resultsRegion.dataset.hallOfChampionsTeamVisibleRecords = String(matches.length);
  resultsRegion.dataset.hallOfChampionsTeamResultsState = hallOfChampionsPeopleArchiveState;
  resultsRegion.setAttribute("aria-busy", String(hallOfChampionsPeopleArchiveState === "loading" && matches.length === 0));

  if ((hallOfChampionsPeopleArchiveState === "idle" || hallOfChampionsPeopleArchiveState === "loading") && matches.length === 0) {
    countElement.textContent = "INDEXING RECORDS";
    const message = document.createElement("p");
    const primaryMessage = document.createElement("span");
    message.className = "hall-of-champions-az-workspace__message hall-of-champions-team-workspace__message";
    message.setAttribute("role", "status");
    primaryMessage.textContent = "INDEXING RECORDS";
    message.append(primaryMessage);
    if (hallOfChampionsPeopleArchiveState === "loading" && hallOfChampionsPeopleArchiveShowConnectionStatus) {
    ...
    message.textContent = "ARCHIVE LINK UNAVAILABLE";
    fragment.append(message);
  } else if (!selectedTeam || matches.length === 0) {
...
function resetHallOfChampionsTeamResultsScroll(prototypeShell) {
  const scrollElement = prototypeShell?.querySelector("[data-hall-of-champions-team-result-scroll]");
  if (scrollElement) {
...
function setHallOfChampionsPeopleArchiveState(prototypeShell, stateName, options = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(options, "error")) {
    hallOfChampionsPeopleArchiveError = options.error;
  }
  if (Object.prototype.hasOwnProperty.call(options, "totalRecords")) {
    hallOfChampionsPeopleArchiveTotalRecords = options.totalRecords;
  }
  if (prototypeShell) {
    prototypeShell.dataset.hallOfChampionsPeopleArchiveState = stateName;
    prototypeShell.dataset.hallOfChampionsPeopleArchiveRecords = String(hallOfChampionsPeopleArchiveRecords.length);
    prototypeShell.dataset.hallOfChampionsPeopleArchivePages = String(hallOfChampionsPeopleArchivePagesRequested);
    prototypeShell.dataset.hallOfChampionsPeopleArchiveTotal = String(hallOfChampionsPeopleArchiveTotalRecords);
  }
...
function requestHallOfChampionsPeopleArchiveData(prototypeShell, options = {}) {
  ...
  if (hallOfChampionsPeopleArchiveRequest) {
    renderHallOfChampionsAzResults(prototypeShell);
...
function handleHallOfChampionsPeopleArchiveRetry(event) {
  event.preventDefault();
...
function syncHallOfChampionsTeamIndex() {
  if (hallOfChampionsPeopleArchiveTeams.length === 0) {
    hallOfChampionsTeamIndex = 0;
    return;
  }
  if (hallOfChampionsTeamIndex < 0 || hallOfChampionsTeamIndex >= hallOfChampionsPeopleArchiveTeams.length) {
    hallOfChampionsTeamIndex = 0;
  }
}
...
function isHallOfChampionsTeamModeActive(prototypeShell) {
  const modeSelector = getHallOfChampionsModeSelector(prototypeShell);
  return Boolean(prototypeShell)
    && prototypeShell.dataset.hallOfChampionsExpanded === "true"
    && Number(modeSelector?.dataset.hallOfChampionsModeIndex || "0") === 3;
}

function syncHallOfChampionsTeamLowerSelectorReadout(prototypeShell, currentTeam = getHallOfChampionsCurrentTeam()) {
  const selector = getHallOfChampionsAzLetterSelector(prototypeShell);
  if (!selector) {
    return;
  }

  const selectedTeam = getWrestlingText(currentTeam).trim();
  const currentLabel = selector.querySelector("[data-hall-of-champions-letter-label='current']");
  const incomingLabel = selector.querySelector("[data-hall-of-champions-letter-label='incoming']");
  const readout = selector.querySelector("[data-hall-of-champions-letter-readout]");
  updateHallOfChampionsLowerSelectorMode(selector, "team");
  selector.dataset.hallOfChampionsTeamIndex = String(hallOfChampionsTeamIndex);
  selector.dataset.hallOfChampionsTeamLabel = selectedTeam;
  selector.dataset.hallOfChampionsTeamLabelSize = selectedTeam.length > 18
    ? "long"
    : selectedTeam.length > 10 ? "medium" : "short";
  if (currentLabel) {
    currentLabel.textContent = selectedTeam;
  }
  if (incomingLabel) {
    incomingLabel.textContent = "";
  }
  if (readout) {
    readout.setAttribute("aria-label", selectedTeam ? `Selected team ${selectedTeam}` : "No team selected");
  }
}

function syncHallOfChampionsTeamWorkspace(prototypeShell) {
  syncHallOfChampionsTeamIndex();
  const currentTeam = getHallOfChampionsCurrentTeam();
  const readout = prototypeShell?.querySelector("[data-hall-of-champions-team-readout]");
  if (prototypeShell) {
    prototypeShell.dataset.hallOfChampionsTeamCount = String(hallOfChampionsPeopleArchiveTeams.length);
    prototypeShell.dataset.hallOfChampionsTeamIndex = String(hallOfChampionsTeamIndex);
    if (currentTeam) {
      prototypeShell.dataset.hallOfChampionsSelectedTeam = currentTeam;
    } else {
      delete prototypeShell.dataset.hallOfChampionsSelectedTeam;
    }
  }
  if (readout) {
    readout.textContent = currentTeam
      ? `CURRENT TEAM // ${currentTeam}`
      : "CURRENT TEAM // DISCOVERING";
  }
  renderHallOfChampionsTeamResults(prototypeShell);
  if (isHallOfChampionsTeamModeActive(prototypeShell)) {
    syncHallOfChampionsTeamLowerSelectorReadout(prototypeShell, currentTeam);
  }
}

function syncHallOfChampionsTeamSelectorWhenActive(prototypeShell) {
  const modeSelector = getHallOfChampionsModeSelector(prototypeShell);
  const isTeamMode = Number(modeSelector?.dataset.hallOfChampionsModeIndex || "0") === 3;
  const canShowTeamSelector = Boolean(prototypeShell)
    && prototypeShell.dataset.hallOfChampionsExpanded === "true"
    ...
    && isTeamMode;
  if (canShowTeamSelector) {
    setHallOfChampionsTeamSelectorAvailable(prototypeShell, true);
  }
}

function updateHallOfChampionsTeamSelectorDisplay(prototypeShell) {
  syncHallOfChampionsTeamWorkspace(prototypeShell);
  syncHallOfChampionsTeamLowerSelectorReadout(prototypeShell);
}
...
function updateHallOfChampionsLowerSelectorMode(selector, modeName) {
  ...
  if (modeName === "team") {
    selector.setAttribute("aria-label", "Hall of Champions team selector");
    previousButton?.setAttribute("aria-label", "Previous team");
    nextButton?.setAttribute("aria-label", "Next team");
    return;
  }
...
function isHallOfChampionsCategoryModeActive(prototypeShell) {
  const modeSelector = getHallOfChampionsModeSelector(prototypeShell);
  return Boolean(prototypeShell)
    && prototypeShell.dataset.hallOfChampionsExpanded === "true"
...
function syncHallOfChampionsCategoryWorkspace(prototypeShell) {
  ...
  renderHallOfChampionsCategoryResults(prototypeShell);
  if (isHallOfChampionsCategoryModeActive(prototypeShell)) {
    syncHallOfChampionsCategoryLowerSelectorReadout(prototypeShell, currentCategory);
  }
}
...
function setHallOfChampionsTeamSelectorAvailable(prototypeShell, isAvailable) {
  const selector = getHallOfChampionsAzLetterSelector(prototypeShell);
  if (!selector) {
    return;
  }

  const hasTeams = hallOfChampionsPeopleArchiveTeams.length > 0;
  const canUseSelector = Boolean(isAvailable) && hasTeams;
  if (canUseSelector) {
    updateHallOfChampionsTeamSelectorDisplay(prototypeShell);
  }

  prototypeShell.dataset.hallOfChampionsTeamSelectorReady = String(canUseSelector);
  if (!canUseSelector) {
    return;
  }
...
function cycleHallOfChampionsTeam(prototypeShell, direction) {
  const selector = getHallOfChampionsAzLetterSelector(prototypeShell);
  if (!selector || prototypeShell?.dataset.hallOfChampionsTeamSelectorReady !== "true") {
    return;
  }
  if (selector.dataset.hallOfChampionsLetterStatus === "changing") {
    return;
  ...
  const totalTeams = hallOfChampionsPeopleArchiveTeams.length;
  if (totalTeams === 0) {
    return;
  }

  const step = direction === "previous" ? -1 : 1;
  const previousTeam = getHallOfChampionsCurrentTeam();
  const nextIndex = (hallOfChampionsTeamIndex + step + totalTeams) % totalTeams;
  const nextTeam = hallOfChampionsPeopleArchiveTeams[nextIndex] || previousTeam;
  const currentLabel = selector.querySelector("[data-hall-of-champions-letter-label='current']");
  const incomingLabel = selector.querySelector("[data-hall-of-champions-letter-label='incoming']");

  hallOfChampionsTeamIndex = nextIndex;
  syncHallOfChampionsTeamWorkspace(prototypeShell);
  resetHallOfChampionsTeamResultsScroll(prototypeShell);
  if (prefersHallOfChampionsReducedMotion() || !currentLabel || !incomingLabel) {
    updateHallOfChampionsTeamSelectorDisplay(prototypeShell);
    return;
  }

  currentLabel.textContent = previousTeam;
  incomingLabel.textContent = nextTeam;
  selector.dataset.hallOfChampionsTeamIndex = String(nextIndex);
  selector.dataset.hallOfChampionsLetterDirection = direction;
  selector.dataset.hallOfChampionsLetterStatus = "changing";

  hallOfChampionsAzLetterTransitionTimer = window.setTimeout(() => {
    completeHallOfChampionsTeamTransition(selector);
  }, HALL_OF_CHAMPIONS_AZ_LETTER_TRANSITION_MS);
}

function completeHallOfChampionsTeamTransition(selector) {
  const prototypeShell = getHallOfChampionsPrototypeShellFromNode(selector);
  if (!selector || !prototypeShell) {
    hallOfChampionsAzLetterTransitionTimer = 0;
    return;
...
function syncHallOfChampionsSearchWorkspace(prototypeShell) {
  ...
  const isTeamReady = canShowWorkspace && activeModeIndex === 3;
  if (isCategoryReady || isTeamReady) {
  ...
  if (isAzReady) {
    ...
    setHallOfChampionsTeamSelectorAvailable(prototypeShell, false);
    ...
    setHallOfChampionsAzLetterSelectorAvailable(prototypeShell, false);
    setHallOfChampionsCategorySelectorAvailable(prototypeShell, false);
    setHallOfChampionsTeamSelectorAvailable(prototypeShell, Boolean(azLetterSelector));
  } else {
    setHallOfChampionsAzLetterSelectorAvailable(prototypeShell, false);
    setHallOfChampionsCategorySelectorAvailable(prototypeShell, false);
    setHallOfChampionsTeamSelectorAvailable(prototypeShell, false);
...
function resetHallOfChampionsModeSelector(prototypeShell) {
  ...
  if (currentLabel) {
    currentLabel.textContent = defaultMode;
  }
  if (incomingLabel) {
    incomingLabel.textContent = "";
  }
...
function completeHallOfChampionsModeTransition(modeSelector, nextIndex) {
  const nextMode = HALL_OF_CHAMPIONS_ARCHIVE_MODES[nextIndex];
  const currentLabel = modeSelector.querySelector("[data-hall-of-champions-mode-label='current']");
  const incomingLabel = modeSelector.querySelector("[data-hall-of-champions-mode-label='incoming']");
  if (currentLabel) {
...
function cycleHallOfChampionsArchiveMode(prototypeShell, direction) {
  ...
  const step = direction === "previous" ? -1 : 1;
  const currentIndex = Number(modeSelector.dataset.hallOfChampionsModeIndex || "0");
  const nextIndex = (currentIndex + step + HALL_OF_CHAMPIONS_ARCHIVE_MODES.length) % HALL_OF_CHAMPIONS_ARCHIVE_MODES.length;
  const nextMode = HALL_OF_CHAMPIONS_ARCHIVE_MODES[nextIndex];
  const currentMode = HALL_OF_CHAMPIONS_ARCHIVE_MODES[currentIndex] || HALL_OF_CHAMPIONS_ARCHIVE_MODES[0];
  const currentLabel = modeSelector.querySelector("[data-hall-of-champions-mode-label='current']");
  const incomingLabel = modeSelector.querySelector("[data-hall-of-champions-mode-label='incoming']");
...
function handleHallOfChampionsAzLetterNavigation(event) {
  ...
  if (selector?.dataset.hallOfChampionsLowerSelectorMode === "team") {
    cycleHallOfChampionsTeam(prototypeShell, direction);
    return;
  }
...
function handleHallOfChampionsAzLetterSelectorKeydown(event) {
  ...
  const prototypeShell = getHallOfChampionsPrototypeShellFromNode(event.currentTarget);
  const selectorMode = event.currentTarget?.dataset.hallOfChampionsLowerSelectorMode;
  const isCategoryMode = selectorMode === "category";
  const isTeamMode = selectorMode === "team";
  const isReady = isCategoryMode
    ? prototypeShell?.dataset.hallOfChampionsCategorySelectorReady === "true"
    : isTeamMode
      ? prototypeShell?.dataset.hallOfChampionsTeamSelectorReady === "true"
...
function handleHallOfChampionsModeSelectorKeydown(event) {
  ...
  event.preventDefault();
  cycleHallOfChampionsArchiveMode(prototypeShell, event.key === "ArrowLeft" ? "previous" : "next");
}
...
function scheduleHallOfChampionsModeSelectorAvailability(prototypeShell) {
  ...
  const revealDelay = prefersHallOfChampionsReducedMotion() ? 0 : HALL_OF_CHAMPIONS_EXPANDED_HOLOGRAM_TRANSITION_MS;
  hallOfChampionsModeSelectorRevealTimer = window.setTimeout(() => {
    if (prototypeShell.dataset.hallOfChampionsActive !== "true" || prototypeShell.dataset.hallOfChampionsExpanded !== "true") {
      return;
    }
